...
Mise à jour finale du 30 août 2026 : **PARTIAL / STOP P1-B sur un défaut frontend réel de synchronisation de version.** La nouvelle recette duo a prouvé les rejeux sans doublon, les refus de clé contradictoire/version périmée et la fidélité d'un brouillon historique non envoyé. Deux vues soumettant une révocation depuis v5 n'ont créé qu'une v6 ; la seconde a été refusée, mais son statut HTTP exact et la latence restent non établis. Les requêtes SQL lancées en parallèle n'ont pas démontré de chevauchement transactionnel : ne pas revendiquer cette preuve. **Après sélection de v2, la carte et la DB indiquent v2 alors que le panneau de préparation conserve v1 et son ancien montant jusqu'au rechargement.** Le backend contient une garde sur la version sélectionnée, vérifiée statiquement, sans tentative de marquage/envoi. Le nouveau sandbox a été intégralement nettoyé : 28 lignes supprimées, aucun fichier, baselines 64 dossiers/162 runs/9 versions/45 brouillons/5 users/0 décision et 35 empreintes hors sandbox identiques. Git/GitHub/Lovable restent `78b08603`, tarifs et migration inchangés. Aucun patch applicatif, commit, push ou publication ; présente roadmap mise à jour localement. **P1-C non commencé : un GO correctif frontend ciblé est nécessaire.**
