...
Mise à jour du 30 août 2026 après reprise : le bundling P1-B est corrigé par `a85bfd5c`, CI complète verte, et les trois fonctions sont déployées. **PARTIAL : contrôles TTC/PAD positifs, STOP sur le parcours de recalcul après versionnement.** La confirmation Chrome a été débloquée par l'utilisateur. Le pricing reprend exactement la commission fournisseur TTC de 3 304 FCFA, sans doublon ni TVA SODATRA additionnelle ; le rejet PAD bloque et sa révocation est enregistrée. Toutefois, `CaseView` masque le recalcul en statut `QUOTED_VERSIONED`, alors que le backend l'autorise. La version historique reste intacte, le PDF est provisoire ; aucun brouillon/email créé. **Nettoyage non effectué : approbation humaine de session Storage requise**, inventaire exact au dernier verdict P1-B. Aucun nouveau patch applicatif ni GO de production générale ; P1-C reste suspendu.
