...
### 3.27 SCENARIO-ALERTS-4-5 — périmètre PAD v3 et danger sur scénarios anciens (24 septembre 2026, publié et recetté)
...
Publication (GO du 24/09) : work fb4cdb0 → da9da0f (b04b4f2 code, 142e7b8 file, da9da0f roadmap), push sans force, Lovable synchronisé ; CI 36034296106 : seul échec Vitest baseline `LocalTransportEstimateFields`, identique au run de fb4cdb0, aucune régression. Déploiement privé de `run-scenario-pricing` seule (Lovable, depuis da9da0f, sondes OPTIONS 200 / POST sans auth 401) ; aucune migration ni publication publique.
Recette runtime sur sandbox synthétiques par l'aperçu authentifié (détail dans la file, entrée du 24/09 17:20 UTC) : bouton v3 actif en import DAP et désactivé hors périmètre avec le motif exact ; run v3 hors périmètre bloqué `SCENARIO_PAD_PRICING_SCOPE_UNSUPPORTED` ; « Revenir en v2 » retire les choix du brouillon sans toucher la révision enregistrée (hash et `pad_choices` inchangés) et la révision v2 se calcule sans blocage de périmètre ; scénario v1 « Colis » : « non » explicite sans code `SCENARIO_DG_*`, « oui » et contradiction bloqués `SCENARIO_DG_NON_CONTAINER_UNSUPPORTED`. Non prouvé : PAD v3 en transit (non ouvert), cas « inconnu » par l'UI (couvert par tests). Observation hors lot : éligibilité v3 lue depuis un cache de 60 s après ajout d'un fait sur la même page.
Reste : alerte 3 (mode terminal par lot en multi-lot) en conception préalable seulement, aucune implémentation. Rollback : revert de b04b4f2 et redéploiement de `run-scenario-pricing` depuis fb4cdb0 ; aucune donnée client touchée.

### 3.28 MULTI-LOT-TERMINAL-1 — confirmations par lot (25 septembre 2026, publié, déployé et recetté ; alerte 3 close)

GO de réalisation locale ; exécutant unique Claude Code, contre-revue indépendante DB/pricing ; base work f060bea. Trace détaillée dans `docs/CTO_GO_QUEUE.md` (entrées alerte 3 et STOP build-case-puzzle du 25/09).
Registre distinct `quote_lot_confirmations` (liaison lot↔ligne par empreinte métier, mode terminal par lot), append-only, sourcé, CAS sur l'empreinte globale ; aucune liaison par position, lignes indiscernables refusées, toute réanalyse périme tout sans réassociation. PAD multi-lot : chaque groupe lié à sa ligne, contrôle d'allocation par ligne, une ligne PAD par décision tous lots confondus, sans PAD global ni moteur. Le fait terminal global ne vaut jamais pour un lot.
Preuves locales : PostgreSQL isolé (schéma seul, synthétique) registre, droits, concurrence réelle, rollback/relivraison et parcours deux lots → clôture PASS ; Deno 1712/1 et Vitest 554/555 (échecs baseline identiques) ; typecheck, lint, typecheck Deno, build, bundles PASS. Contre-revue : B1 (double lecture du registre) corrigé.
GO complémentaire : exception FROZEN bornée dans build-case-puzzle (gaps terminal/PAD dossier levés seulement si ≥ 2 lignes persistées et recomptées) ; parcours sur handlers réels et moteur réel (double Supabase psql, IA synthétique) 10/10, référence f060bea bloquée en NEED_INFO ; concurrence réelle réécriture des lignes / clôture : enregistrement cohérent ou refus. NOT_RUN : Cloud, PostgREST/GoTrue, runtime Lovable, IA réelle, UI navigateur.
Publication (GO du 25/09) : f060bea → b8f76d2 poussé ; Lovable a ajouté b16284a et 96cddce (types régénérés) ; CI 36155640964 échec Vitest `LocalTransportEstimateFields` = baseline, étapes suivantes NOT_RUN en CI. Migration `20260925120000` appliquée sur Lovable Cloud (208 entrées, MD5 fichier et fonctions identiques, RLS active, registre vide) ; run-pricing, manage-pad-group-confirmation, manage-lot-confirmation puis build-case-puzzle déployés (OPTIONS 200, POST sans auth 401).
Recette déployée sur sandbox synthétique d937116b… (détail dans la file, entrée du 25/09 17:40 UTC) : refus sans confirmation PASS ; scénario, liaisons, LoLo et PAD confirmés depuis l'interface PASS ; calcul **FAIL** (`PAD_GROUP_CONFIRMATION_REQUIRED` par lot) : `lotPadAllocationIssue` exige un tableau JSON alors que l'extraction réelle écrit `cargo.containers` en texte (« 2x40HC ») ; réanalyse → confirmations périmées et calcul bloqué PASS.
Correctif (GO option B du 25/09) : 72fd67b → e9bdf6a, lecteur unique `readLotContainers` pour l'allocation PAD et le calcul par lot (JSON existant et texte strict « 2x40HC ») ; valeur présente mais illisible → lot bloqué `LOT_CONTAINERS_UNREADABLE` avant le moteur ; mono-lot identique à 72fd67b. Parcours local 16/16 en JSON et texte (équivalence par lot normalisée). CI 36178269964 = baseline Vitest. Mêmes quatre fonctions redéployées, aucune migration.
Recette déployée d937116b… PASS : run 4 `success` (lot 1 2x40HC, lot 2 1x20DV ; Terminal, Transport, Surestaries et un PAD par lot, 348 408 / 16 992 FCFA, sans cumul ; HT 1 798 160 XOF) ; réanalyse → run 5 refusé ; reconfirmation → run 6 identique. Alerte 3 close. Limites : Terminal (DPW) à confirmer (famille DTHC), réanalyse lancée par l'appel du bouton (masqué en PRICED_DRAFT). Aucune écriture hors sandbox. Rollback : fonctions depuis f060bea puis script SQL avec accusé ; registre archivé, jamais supprimé ni réactivé.
Héritage des conteneurs (GO du 26/09) : b1e6941 → f626c37, `lotPricingContainers` : un lot n'est jamais calculé avec les conteneurs du dossier ni d'un autre lot ; sans valeur propre, aucun conteneur seulement si indice et forfait sont LCL/aérien, sinon `LOT_CONTAINERS_REQUIRED`. Défaut prouvé sur b1e6941 (lot sans conteneurs chiffré avec 3x20DV du dossier hors PAD), corrigé en local 19/19 (JSON/texte), mono-lot identique. CI 36235639787 = baseline ; mêmes quatre fonctions redéployées, sans migration. Cloud : fixture synthétique aa01530d… (lot 2 sans conteneurs, fait dossier 2x40HC) → lot 2 bloqué `LOT_CONTAINERS_REQUIRED` ; d937116b… run 7 identique au run 6. Limite : hors PAD prouvé en local seulement.
