...
### 3.19 Lot IMO-RULES-1 — classification IMDG livrée, table de séjour BLOQUÉE (10 septembre 2026)

GO CTO « go IMO-RULES-1 ». Trois sous-lots livrés : `56433911` (nomenclature et faits), `ba1cc3d3` (cohérence avec DG-1), `e3028264` (saisie opérateur).
...
**BLOQUÉ — le cœur du lot.** La table de référence de l'**Annexe 1 v4.0** (mode de séjour, franchise 0/3 jours au lieu de 15) n'est disponible nulle part : ni dans le dépôt, ni parmi les 5 documents tarifaires en base, ni parmi les 45 documents chargés. Encoder des franchises de séjour non sourcées aurait un impact financier direct sur les surestaries facturées au client. **Statut : en attente que le document soit fourni.** Aucune valeur de franchise, de surcharge ou de mode de séjour n'a été écrite.
