...
Mise à jour du 30 août 2026 : le blocage de bundling P1-B est corrigé par `a85bfd5c`, CI complète verte, et les trois fonctions concernées sont déployées sur Lovable. **PARTIAL : recette métier non terminée**, interrompue par une boîte de confirmation native Chrome que le contrôle du navigateur ne parvient pas à accepter. Un dossier et un document purement synthétiques existent et restent à nettoyer après reprise ; aucun pricing, décision maritime, version ou email n'a été produit pour cette recette. P1-B n'est pas validé runtime et P1-C n'a pas commencé. Voir le dernier verdict de la section P1-B avant toute reprise. Aucun GO de production générale n'est donné.
