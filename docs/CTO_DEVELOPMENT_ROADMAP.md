...
Recette déployée d937116b… PASS : run 4 `success` (lot 1 2x40HC, lot 2 1x20DV ; Terminal, Transport, Surestaries et un PAD par lot, 348 408 / 16 992 FCFA, sans cumul ; HT 1 798 160 XOF) ; réanalyse → run 5 refusé ; reconfirmation → run 6 identique. Alerte 3 close. Limites : Terminal (DPW) à confirmer (famille DTHC), réanalyse lancée par l'appel du bouton (masqué en PRICED_DRAFT). Observation hors lot en PENDING : lot sans conteneurs propres hérite du fait dossier. Aucune écriture hors sandbox. Rollback : fonctions depuis f060bea puis script SQL avec accusé ; registre archivé, jamais supprimé ni réactivé.
