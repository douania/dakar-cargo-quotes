...
Aucune écriture Cloud, migration, Auth/RLS, tarif, version, brouillon ou envoi au contrôle local. UI inchangée (complétude/relance analyse hors GO). Déploiement et recette runtime du nouveau contrôle NOT_RUN à la préparation du commit ; livraison privée autorisée, avec module embarqué et vérification des empreintes avant/après.
Rollback local : retirer uniquement ce patch et les deux fichiers ajoutés en préservant la roadmap antérieure ; après livraison éventuelle, revert du commit du lot et redéploiement de run-pricing, sans restauration de faits.
