...
#### PAD v3 — livraison privée — 15 septembre 2026, 16:16 UTC — SQL PASS / Edge et recette NOT_RUN
...
- Contrôle bundles interrompu ENOENT (deno absent PATH), livraison suspendue ; diagnostic trouve Deno2.9.5 déjà installé dans cache npm, aucune installation ni correction code. Rejeu avec DENO_BIN explicite PASS39/43/34 et contrôle négatif ; config94/diff-check PASS. Suite applicative complète non relancée, preuves du lot local conservées.
- Statut à ce point : SQL livré, commit/push/Edge/recette à poursuivre sous le même GO. Ne créer aucun scénario v3 pendant la fenêtre ; rollback v2 uniquement à zéro historique v3, ledger conservé et nouvelle version obligatoire après retour arrière. Pas de publication publique, faits/barèmes/Auth/RLS inchangés.
