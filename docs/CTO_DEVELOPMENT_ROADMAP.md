...
Mise à jour du 30 août 2026 : bundling P1-B corrigé par `a85bfd5c`, trois fonctions déployées, contrôles TTC/PAD positifs. **PARTIAL runtime ; correctif frontend de reprise après versionnement validé localement sous GO ciblé.** Ce lot réaffiche le recalcul manuel en `QUOTED_VERSIONED`, maintient les états envoyés/clos verrouillés et rafraîchit l'alerte après chaque tentative ; 271 tests frontend et 837 tests Deno réussis (6 ignorés), sept gates locaux verts. Publication, CI GitHub et recette de ce lot restent à vérifier. La version historique demeure intacte, le PDF est provisoire ; aucun brouillon/email créé. **Nettoyage non effectué** : Storage est désormais accessible via la session plateforme Lovable existante, sans minting ; confirmation normale requise avant suppression des seuls objets synthétiques. Aucun GO de production générale ; P1-C reste suspendu jusqu'au PASS runtime P1-B et nettoyage.
