...
## [2026-09-26 13:10 UTC] PENDING — Arbitrage multi-lot : poids par conteneur et famille DTHC hérités du dossier
**Origine** : GO utilisateur « arbitrer poids par conteneur et famille DTHC » (26/09) ; diagnostic lecture seule, base c912423. Aucune modification.
**Constat vérifié** : (1) les lignes multi-lot ne peuvent porter ni `cargo.weight_per_container_kg` ni `pricing.dthc_family` (hors `MULTI_QUOTE_ALLOWED_KEYS`) ; chaque lot reçoit donc les faits du dossier, au moteur (`run-pricing` l. 2123-2126 via `lc.inputs`) comme dans `price-service-lines` (contexte lu en base l. 470-473 ; l'override par lot l. 2245-2259 ne remplace ni `dthc_family`, ni `weight_per_container_kg`, ni `cargo_description`). (2) Poids : un poids par conteneur explicite prime sur la dérivation poids ÷ boîtes (`deriveCargoWeightPerContainerKg`) et décide la règle TRUCKING-22T (20' > 22 t facturé au tarif 40'). Sandbox d937116b… : fait dossier 18 000 kg (lot 1) appliqué au lot 2 (1x20DV, 12 000 kg réels) ; sans effet sur le montant ici (< 22 t des deux côtés) mais sur/sous-facturation possible dès qu'un lot franchit 22 t. (3) Famille : le fait dossier (choix opérateur) s'applique à tous les lots, y compris `isIMO` via `resolveDangerousForEngine` (l. 174) ; sans fait, `price-service-lines` infère depuis la désignation du dossier (« pièces détachées » appliquée au lot mobilier) alors que le moteur utilise celle du lot. Aucune famille par lot n'existe.
**Options** : (a) poids : en multi-lot, ignorer le fait dossier explicite et dériver depuis le poids et les conteneurs propres au lot (règle existante ; inconnu ⇒ tarif 20' avec mention, doctrine inchangée) ; (b) famille : fait dossier présent et lot en périmètre DTHC ⇒ blocage `LOT_DTHC_FAMILY_REQUIRED` (jamais attribuée silencieusement) ; absent ⇒ inférence depuis la désignation propre du lot, moteur et `price-service-lines` alignés ; (c) plus tard, décision de famille par lot dans le registre des lots (migration + UI) pour débloquer (b).
**Recommandation de Claude Code** : lot court (a)+(b), sans migration, exception ciblée run-pricing (préparation par lot + override `price-service-lines`), mono-lot inchangé, parcours local avec deux lots de poids/famille différents ; (c) sur GO séparé si le blocage (b) gêne en pratique. Observation voisine à trancher avec (a) : un lot sans `cargo.weight_kg` propre hérite aussi du poids total du dossier (d937116b… : 18 000 kg).

---

## [2026-09-26 12:40 UTC] TRAITÉ — Suppression de l'héritage des conteneurs du dossier en multi-lot (f626c37) publiée, déployée et recettée
**Origine** : GO dédié du 26/09 (entrée PENDING ci-dessous) ; exécutant unique Claude Code ; contre-revue indépendante lecture seule (GO avec réserves, deux réserves appliquées).
**Correctif** : `_shared/lot-confirmation.ts` `lotPricingContainers` : un lot n'est calculé qu'avec ses propres conteneurs lisibles ; sans valeur propre, aucun conteneur seulement si l'indice (SEA_LCL_IMPORT, AIR_IMPORT, AIR_LCL_IMPORT) ET le forfait résolu (LCL_/AIR_IMPORT_*) sont non conteneurisés ; sinon `LOT_CONTAINERS_REQUIRED` (FCL, projet, indice inconnu, contradiction indice/forfait). `run-pricing` (boucle multi-lot) ne transmet plus jamais la valeur fusionnée du dossier ; lecture stricte `readLotContainers` inchangée ; mono-lot inchangé ; aucune formule, tarif, donnée persistée ni migration.
**Contrôles** : tests Deno ciblés 89/89 ; suite Deno 1721/1 (baseline `set_intake_facts_batch.test.ts:191`) ; typecheck Deno 49/5, lint 732/16, eslint, bundles, config PASS ; frontend non touché (Vitest, typecheck et build frontend NOT_RUN localement, couverts par la CI). Parcours local sur handlers réels 19/19 en JSON et en texte : hors PAD, deux lots dont un seul a ses conteneurs et un fait dossier différent (3x20DV) → lot B FCL bloqué `LOT_CONTAINERS_REQUIRED`, aucun run calculé ; indice inconnu bloqué, indice absent refusé par la garde existante `LOT_REQUEST_TYPE_REQUIRED` ; lot AIR_IMPORT → run `success`, moteur : lot 1 = 2x40HC, lot 2 = [] ; périmètre PAD → lot sans conteneurs bloqué. Sonde sur b1e6941 : le même cas réussissait avec lot B = 3x20DV du dossier (défaut prouvé). Mono-lot identique à b1e6941 (JSON et texte).
**Publication** : b1e6941 → f626c37 ; CI 36235639787 : seul échec Vitest `LocalTransportEstimateFields` = baseline. Déploiement Lovable depuis f626c37 de run-pricing, manage-pad-group-confirmation, manage-lot-confirmation, build-case-puzzle ; sondes OPTIONS 200, POST sans auth 401. Aucune migration.
**Recette Cloud (synthétique, session authentifiée, interface)** : fixture n° 2 (fil a1c0f7e2…, e-mail b7e4d2c9…, @example.test, lot 2 sans nombre ni type de conteneurs) → « Traiter » → dossier aa01530d… READY_TO_PRICE ; l'IA réelle n'a donné aucun `cargo.containers` au lot 2 (SEA_FCL_IMPORT) alors que le fait dossier vaut 2x40HC (celui du lot 1) ; run 1 `blocked` : lot 2 `LOT_CONTAINERS_REQUIRED` (+ exigences de confirmation), lot 1 sans ce code, aucune requête moteur. Non-régression d937116b… : run 7 `success` identique au run 6 (mêmes lignes, montants, conteneurs envoyés, HT 1 798 160 / TTC 1 996 160 XOF).
**Écritures sandbox** : fil, e-mail et dossier aa01530d… (analyse : faits et 2 lignes), 1 run `blocked` ; d937116b… : 1 run `success`. Hors périmètre depuis 09:00 UTC : 0 run, mise à jour, job, décision ; 0 brouillon, version, document.
**Limites** : cas hors PAD prouvé en local seulement (le dossier Cloud synthétique est dans le périmètre PAD) ; `weightPerContainerKg` et `dthcFamily` restent lus depuis le fait dossier fusionné (hors conteneurs, non modifiés, à arbitrer si besoin) ; aucun libellé d'interface dédié pour `LOT_CONTAINERS_REQUIRED` (code affiché tel quel, comme les autres codes de lot).

---

## [2026-09-25 20:25 UTC] TRAITÉ (résolu par f626c37, entrée du 26/09 12:40 UTC) — Lot multi-lot sans fait `cargo.containers` propre : hérite du fait dossier (observation hors lot, arbitrage)
**Origine** : contre-revue du correctif e9bdf6a (réserve 2), vérifiée dans le code.
**Constat** : dans `run-pricing` (boucle multi-lot), un lot dont la ligne ne porte aucun `cargo.containers` garde le chemin fusionné `mergeFactsForLot` et hérite du fait dossier, qui en multi-lot ne décrit qu'un lot (sur le sandbox : le dernier, 1x20DV). Dans le périmètre PAD par groupes, le contrôle d'allocation (fait de ligne exigé) bloque ce lot ; hors de ce périmètre (ex. terminal seul), il peut être chiffré avec les conteneurs d'un autre lot. Préexistant, non modifié par e9bdf6a (le GO visait « présent mais illisible »).
**Options** : (a) bloquer un lot conteneurisé (indice FCL) sans conteneurs propres ; (b) conserver et documenter. Aucune modification faite.
**Recommandation de Claude Code** : (a), bornée à la boucle multi-lot, avec parcours local JSON/texte/absent.

---

## [2026-09-25 20:25 UTC] TRAITÉ — Alerte 3 close : lecture stricte partagée des conteneurs par lot (e9bdf6a) publiée, déployée et recettée
**Origine** : GO correctif ciblé puis GO complémentaire option B (25/09) ; exécutant unique Claude Code ; contre-revues indépendantes lecture seule (deux, limitées au correctif).
**Correctif** : `_shared/lot-confirmation.ts` `readLotContainers` (lecteur unique pour l'allocation PAD et le calcul par lot : liste JSON non vide existante conservée, entrées avec type et quantité entière > 0 ; texte strict à un seul groupe « 2x40HC » ; tout autre contenu présent = illisible) ; `run-pricing` (exception ciblée, boucle multi-lot) : conteneurs propres au lot transmis au moteur, valeur présente mais illisible → lot bloqué `LOT_CONTAINERS_UNREADABLE` avant le moteur ; absent et mono-lot inchangés ; aucun fait réécrit, aucune formule, tarif, moteur ni migration.
**Contrôles** : tests Deno ciblés 88/88 ; suite Deno 1720/1 (échec baseline `set_intake_facts_batch.test.ts:191`) ; Vitest 554/555 (baseline) ; typecheck, typecheck Deno 49/5, lint 732/16, config, bundles, build PASS. Parcours local sur handlers réels, 16/16 en JSON et en texte : équivalence normalisée par lot (prestations, quantités, montants, devises, totaux, réserves, références tarifaires, conteneurs envoyés au moteur), Terminal (DPW), Transport, Surestaries et une ligne PAD par lot ; réanalyse, refus, reconfirmation ; cas illisibles (texte libre, plusieurs groupes, objet JSON, quantité nulle, sans type, mot en trop, liste vide, fait dupliqué) bloqués ; mono-lot identique au code 72fd67b (sortie normalisée égale).
**Publication** : 72fd67b → e9bdf6a poussé ; CI 36178269964 : seul échec Vitest `LocalTransportEstimateFields` = baseline, étapes suivantes NOT_RUN en CI. Déploiement Lovable depuis e9bdf6a de run-pricing, manage-pad-group-confirmation, manage-lot-confirmation, build-case-puzzle (seules fonctions embarquant le module) ; sondes OPTIONS 200, POST sans auth 401. Aucune migration.
**Recette déployée (sandbox d937116b…, session authentifiée, interface)** : liaisons, LoLo et PAD reconfirmés (v2) sans écart d'allocation ; run 4 `success` : lot 1 = 2x40HC, lot 2 = 1x20DV transmis au moteur ; par lot Terminal (DPW) sur son conteneur, Transport (250 160 / 82 600 XOF), Surestaries, PAD unique (T02 348 408 / T03 16 992 FCFA), agence et douane ; 2 lignes PAD au total, aucun cumul ; HT 1 798 160, TTC 1 996 160 XOF. Réanalyse (lignes réécrites 19:44 et 19:56) : décisions affichées « ancienne valeur, non appliquée », run 5 `blocked` (LOT_LINE_UNBOUND, TERMINAL_OPERATION_MODE_REQUIRED, PAD_GROUP_CONFIRMATION_REQUIRED par lot) ; reconfirmation (v3) ; run 6 `success` identique au run 4.
**Écritures sandbox** : 12 décisions de lot et 6 décisions PAD (d937116b… seulement), 6 runs (4 `blocked`, 2 `success`), 3 jobs d'analyse, lignes réécrites, statut PRICED_DRAFT. Hors périmètre : 0 décision, run, job ou mise à jour d'un autre dossier depuis 16:00 UTC ; 0 brouillon, version, document ; seul e-mail créé = fixture.
**Limites** : Terminal (DPW) reste « à confirmer » (famille DTHC non déterminable depuis la désignation, règle moteur inchangée) ; Surestaries à confirmer (armateur inconnu). En PRICED_DRAFT l'interface masque « Relancer l'analyse » : la réanalyse a été lancée depuis la page par le même appel que le bouton (`backfill-case-documents` puis `build-case-puzzle` start, session utilisateur) ; onglet Edge en arrière-plan sujet à des gels réseau (rechargements). Montants Terminal/Transport locaux nuls faute de barème local (équivalence prouvée ; montants réels prouvés sur Cloud). Nouveau refus volontaire : texte libre ou JSON sans quantité sur un lot = `LOT_CONTAINERS_UNREADABLE`. Observation hors lot séparée (entrée PENDING ci-dessus) ; défauts de contraintes `quote_gaps`/`quote_facts` toujours séparés.

---

## [2026-09-25 17:40 UTC] TRAITÉ (résolu par e9bdf6a, entrée du 25/09 20:25 UTC) — MULTI-LOT-TERMINAL-1 publié et déployé ; recette déployée FAIL à l'étape « calcul réussi » (allocation PAD : conteneurs de ligne en texte)
**Origine** : GO de publication et de recette du 25/09 ; exécutant unique Claude Code, session interactive.
**Publication** : commit b8f76d2 (code, exception build-case-puzzle incluse) poussé sans force, f060bea → b8f76d2 ; Lovable a ensuite poussé b16284a et 96cddce (types Supabase régénérés pour le registre, +101 lignes ; `previewAuthStorage.ts` plateforme), work = origin/work = 96cddce. CI run 36155640964 : seule étape en échec « Frontend tests (Vitest) », `LocalTransportEstimateFields` = baseline (runs 36038557209 et 36034296106 identiques) ; étapes Deno, lint baseline et build NOT_RUN en CI après cet échec, couvertes en local.
**Migration Cloud** : `20260925120000_quote_lot_confirmations.sql` appliquée par bloc SQL unique (garde MD5 du fichier et du nombre d'entrées du registre des migrations, corps et inscription atomiques) : 208 entrées, MD5 égal au fichier (9b896931…) ; définitions des 5 fonctions identiques au local (MD5) ; RLS active ; aucun droit anon/authenticated ; registre vide ; `pad_group_confirmations` 7 lignes inchangées.
**Déploiement** : run-pricing, manage-pad-group-confirmation, manage-lot-confirmation, puis build-case-puzzle en dernier ; sondes OPTIONS 200, POST sans auth 401 ; panneau des lots visible sur le dossier multi-lot.
**Recette (sandbox synthétique, session authentifiée, aucun dossier client ni envoi)** : e-mail fictif cfc345c3… (fil 76d15b75…, expéditeur @example.test) → dossier d937116b… par « Traiter » ; analyse IA réelle : 2 lignes, READY_TO_PRICE, aucun gap ouvert. (1) Calcul sans confirmation : run 1 `blocked` MULTI_LOT_BLOCKED, par lot PAD_CATEGORY_REQUIRED / LOT_SCENARIO_REQUIRED / TERMINAL_OPERATION_MODE_REQUIRED — PASS. (2) Depuis l'interface : scénario v3 bd42b039… (lot-1 2×40hc 18 000 kg/u SOC T02 ; lot-2 1×20dv 12 000 kg SOC T03) sélectionné ; liaisons lot↔ligne et LoLo par lot enregistrées (4 décisions, auteur = session utilisateur) ; PAD T02 et T03 confirmés — PASS. (3) Calcul : run 2 `blocked`, chaque lot `PAD_GROUP_CONFIRMATION_REQUIRED` seul (scénario et terminal reconnus) — **FAIL**. Le panneau PAD affiche `LOT_PAD_ALLOCATION_MISMATCH` pour les deux groupes. (4) Réanalyse : confirmations affichées « ancienne valeur, non appliquée », run 3 `blocked` LOT_LINE_UNBOUND / TERMINAL_OPERATION_MODE_REQUIRED / PAD_GROUP_CONFIRMATION_REQUIRED par lot — PASS (sans reconfirmation, le calcul reste bloqué ; le succès après reconfirmation n'est pas atteignable, cf. cause).
**Étape fautive, cause vérifiée** : `lotPadAllocationIssue` (`_shared/lot-confirmation.ts:241-251`) exige pour `cargo.containers` un tableau JSON à une entrée ; l'extraction IA réelle écrit un texte (`"2x40HC"`, `"1x20DV"`, `valueType: text`, relu en base avant et après réanalyse) → échec de `JSON.parse` → écart d'allocation → aucune ligne PAD par lot. Le parcours local passait car l'IA synthétique produisait le tableau JSON. Aucun correctif appliqué (hors GO).
**Écritures du sandbox** : fil, e-mail, dossier d937116b… ; faits (28 lignes, 14 courantes) et 2 lignes de demande réécrites à la réanalyse ; 1 scénario, 1 sélection ; 4 décisions de lot ; 2 décisions PAD ; 3 runs `blocked`. Hors périmètre : 0 décision de lot sur un autre dossier, 0 run ou mise à jour d'un autre dossier depuis 16:00 UTC, 0 brouillon, version, document ou historique de cotation ; statut du dossier inchangé (READY_TO_PRICE).
**Changement proposé (GO requis)** : dans `lotPadAllocationIssue` seulement, accepter aussi une valeur texte à une seule entrée `N x TYPE` analysée strictement (même sémantique que `parseContainersFromText` de build-case-puzzle, sans l'importer), refuser plusieurs entrées ou un texte ambigu ; tests unitaires sur les formes réelles ; redéploiement de run-pricing, manage-pad-group-confirmation, manage-lot-confirmation (importeurs) ; reprise de la seule étape (3) puis (4) avec reconfirmation sur le même sandbox. Aucune migration.
**Observations séparées (préexistantes, hors lot)** : `quote_gaps_gap_category_check` refuse `gap_category = 'request'` ; `quote_facts_source_type_check` heurte deux faits dérivés du build.
**Rollback** : non déclenché (état intermédiaire fermé : lots bloqués, aucun calcul produit) ; procédure inchangée (fonctions depuis f060bea, puis script SQL avec accusé).

---

## [2026-09-24 17:20 UTC] TRAITÉ — Lot alertes Lovable 4 et 5 : périmètre PAD v3 fidèle au serveur et danger sur scénarios anciens (publié et recetté le 24/09)
...
**Publication (GO du 24/09, 18:05 UTC)** : commits b04b4f2 (code) + 142e7b8 (file) + da9da0f (roadmap) poussés sans force, fb4cdb0 → da9da0f sur work = origin/work ; Lovable synchronisé sur da9da0f. CI GitHub run 36034296106 : seule étape en échec « Frontend tests (Vitest) », `LocalTransportEstimateFields` = baseline (run de fb4cdb0 identique) → aucune régression ; les étapes suivantes ne s'exécutent pas en CI après cet échec (ordre du workflow, comme documenté), couvertes en local. Déploiement privé de la seule fonction modifiée `run-scenario-pricing` par Lovable depuis da9da0f (préflight HEAD/arbre propre/diff limité aux 4 fichiers attendus ; accusé « Successfully deployed edge functions: run-scenario-pricing » ; OPTIONS 200, POST sans auth 401). Aucune migration, publication publique ni changement tarifaire ; SHA Lovable inchangé après déploiement.
**Recette runtime (sandbox synthétiques 9b606375… et 8a06251d…, mécanismes applicatifs authentifiés, aperçu Lovable ; aucun dossier client, envoi, version, PDF ni suppression)** : (1) PAD v3 : sandbox 0d2b54c6… (incoterm « FOB Shanghai ») → bouton v3 désactivé, motif exact « incoterm FOB SHANGHAI : calcul PAD par groupe limité à DAP » ; sandbox 9b606375… après ajout du fait `routing.incoterm = DAP` → bouton actif, scénario v3 dfad6261… enregistré (T05 sur lot-1), run 1 sans `SCENARIO_PAD_PRICING_SCOPE_UNSUPPORTED` (réserve `SCENARIO_DAP_SERVICES_ONLY`, bloqué seulement par type de conteneur et propriété non renseignés) ; incoterm passé à FOB → run 2 bloqué avec `SCENARIO_PAD_PRICING_SCOPE_UNSUPPORTED` et le message précisé affiché ; formulaire de révision : alerte « Ces choix PAD par groupe ne pourront pas être calculés : incoterm FOB : calcul PAD par groupe limité à DAP. Le serveur refusera ce scénario plutôt que d'ignorer les choix. » (2) Retour v2 : confirmation « Retirer les 1 choix PAD de ce brouillon et revenir en v2 ? … », section PAD retirée du brouillon, bouton v3 réaffiché désactivé ; après annulation, révision dfad6261… inchangée en base (même `scope_hash`, schema_version 3, `pad_choices` présents, `updated_at` inchangé) ; nouvelle révision v2 bdc219a7… enregistrée (sans clé `pad_choices`), l'ancienne conservée en « Remplacée » ; run de la v2 sans blocage de périmètre PAD. (3) Scénario v1 lot « Colis » (214641b1…) sur 8a06251d… : `cargo.dangerous_goods = NO` → run 1 sans aucun code `SCENARIO_DG_*` (bloqué uniquement par les faits dossier absents : package, destination, incoterm, valeur, périmètre cargo) ; `= YES` → run 2 `SCENARIO_DG_NON_CONTAINER_UNSUPPORTED` avec le message adapté ; `NO` + `cargo.imo_class = 3` (contradictoire) → run 3 même blocage. Cas « inconnu » non atteignable par l'UI (sélecteur limité à Oui/Non ; couvert par `domain_test.ts`).
**Limites** : aucune preuve PAD v3 en transit (non ouvert par l'UI) ; après ajout d'un fait sur la page, l'éligibilité v3 reste lue depuis un cache de 60 s (`quote-scenario-pad-scope-facts` non invalidé par `set-case-fact`) jusqu'au rechargement — observation hors lot, sans effet serveur. Les faits, scénarios et runs de recette restent sur les sandbox. Clôture documentaire IMO (§3.26 et entrées du 23-24/09) préservée telle quelle.

---

## [2026-09-25 13:30 UTC] TRAITÉ (GO complémentaire du 25/09 ; publié dans b8f76d2, déployé le 25/09) — STOP ciblé MULTI-LOT-TERMINAL-1 : build-case-puzzle (FROZEN) bloque le statut calculable des dossiers multi-lots à package DTHC
**Résolution locale** : exception FROZEN bornée dans `build-case-puzzle/index.ts` (+40/−3) : `perLotGapScopeEligible` (extraction `ai_extraction`, ≥ 2 lignes stockées, recomptage en base identique) ; si éligible, le gap `routing.terminal_operation_mode` ne dépend plus du fait dossier et `pricing.pad_category` n'est plus ouvert ni synchronisé par le build ; un gap ouvert est résolu avec la raison `multi_lot_per_lot_confirmation`. Sinon (mono-lot, échec d'écriture ou de relecture, écart) : comportement inchangé. Aucun mode global prêté aux lots ; autres gaps inchangés ; exigences par lot toujours imposées par run-pricing.
**Preuves** : tests de module build-case-puzzle 15/15 (éligibilité, câblage). Parcours sur handlers réels (build-case-puzzle sync, manage-lot-confirmation, manage-pad-group-confirmation, run-pricing, quotation-engine, price-service-lines) via `scripts/lot-local-handlers/` : PostgreSQL local hors réseau, client Supabase remplacé par un double psql, IA synthétique déterministe, scénario via la RPC SQL `manage_quote_scenario` : 10/10 (analyse → READY_TO_PRICE ; blocage par lot sans décision ; décisions via Edge ; calcul réel avec PAD 3600/1800 par lot et PRICED_DRAFT ; réanalyse périmée ; multi→mono→multi ; échec de persistance non éligible ; autre gap bloquant conservé ; mono-lot inchangé). Référence f060bea sur le même dossier : NEED_INFO avec gaps terminal et PAD bloquants. Concurrence réelle `replace_quote_request_lines` / `complete_lot_pricing` (deux sessions) : enregistrement sur le contexte lu ou refus `LOT_CONTEXT_CHANGED`, aucun écrivain à étendre.
**Contre-revue limitée (exception + concurrence)** : GO avec réserves, aucun blocage. Corrigé ensuite : une décision Edge ne rouvre plus jamais le gap PAD dossier d'un multi-lot (`syncPadGroupGapAfterDecision`, build et rapprochement de poids inchangés) ; cas T4 ajouté (réécriture entre la vérification du wrapper et la relecture du finaliseur PAD : refus `PAD_CONTEXT_CHANGED`). Déclaré : nouvelle raison de timeline `multi_lot_per_lot_confirmation` ; le recomptage ne voit pas une réécriture concurrente de même cardinalité, sans effet sur la cohérence (empreinte globale).
**Observations hors lot (préexistantes, non corrigées)** : le gap `request.multi_lot_unresolved` du build utilise `gap_category = 'request'`, refusée par `quote_gaps_gap_category_check` (NEED_INFO reste forcé) ; deux faits dérivés du build (`cargo.chargeable_weight_*`, port de destination inféré) heurtent `quote_facts_source_type_check`, non bloquant.
**NOT_RUN** : Cloud, PostgREST/GoTrue réels, runtime Edge Lovable, IA réelle, Edge `manage-quote-scenario`, interface navigateur.
**Origine** : réalisation locale MULTI-LOT-TERMINAL-1 (GO CTO du 25/09, point 3), constat confirmé par la contre-revue indépendante.
**Type** : arbitrage requis, composant FROZEN non nommé par le GO. Aucune modification faite.
**Constat vérifié** : chaque build multi-lots réextrait et réécrit `quote_request_lines` (DELETE+INSERT, `build-case-puzzle/index.ts:6432`, `:6490`) ; l'empreinte globale change, toutes les confirmations deviennent périmées. Avant la décision de statut, le build rouvre le gap bloquant `routing.terminal_operation_mode` sur le seul fait dossier (`:7903`) et `pricing.pad_category` (`:7750`) ; `run-pricing` refuse NEED_INFO et tout gap bloquant. Une synchronisation ciblée des gaps ne rend donc pas le parcours utilisable pour un dossier multi-lots dont le package dossier inclut DTHC/PAD : le statut resterait NEED_INFO et tout rebuild ré-invalide tout. Le parcours est utilisable quand le dossier atteint un statut calculable sans rebuild après confirmation (ex. aucun package dossier en périmètre DTHC).
**Changement exact proposé** : dans `build-case-puzzle`, uniquement quand `multiQuoteResult.stored >= 2` : (1) TERMINAL-GAP ne dépend plus du fait dossier ; le gap `routing.terminal_operation_mode` n'est pas ouvert et un gap ouvert est résolu avec la raison `multi_lot_per_lot_confirmation` ; (2) PAD-SCOPE-GAP n'ouvre ni ne maintient `pricing.pad_category` en bloquant pour ce motif. L'exigence reste appliquée par lot dans `run-pricing` (blocage MULTI_LOT_BLOCKED avec codes LOT_*/TERMINAL/PAD) ; le fait terminal global ne résout jamais une exigence par lot. Tests : `terminal-operation-gap_test.ts` et gap PAD multi-lot.
**Risques** : dossier affiché READY_TO_PRICE alors que des lots restent à confirmer (même doctrine que les autres bloqueurs par lot) ; modification d'un module FROZEN.
**Recommandation de Claude Code** : GO borné sur ce seul changement, dans le même lot avant publication ; sinon publier en déclarant la limite.

---

## [2026-09-24 17:45 UTC] CLOS (25/09, b8f76d2 + e9bdf6a) — Alerte Lovable 3 : devis multi-lots conteneurs bloqués (mode terminal par lot) — recette déployée PASS (entrée du 25/09 20:25 UTC)
...
**Verdict CTO (24/09, 18:05 UTC)** : conception préalable uniquement, aucune implémentation autorisée. Proposition de conception livrée en session (MULTI-LOT-TERMINAL-1 : identité stable des lots par empreinte de contexte, confirmations sourcées par lot avec révocation et invalidation au changement de lot, mode terminal par lot, levée bornée de `PAD_MULTI_LOT_UNSUPPORTED` sans suppression de garde, parcours opérateur jusqu'au calcul ; comparaison réutilisation du registre PAD vs nouveau registre ; risques et critères de réussite). Reste PENDING jusqu'au GO de conception.
**Décisions CTO (25/09)** : D1, D3, D4, D5 acceptées ; D2 acceptée avec correction (liaison explicite, jamais par position, aucune interchangeabilité, lignes indiscernables bloquées, aucune réassociation silencieuse). GO de réalisation locale sur le périmètre final A à D.
**Réalisation locale (25/09, non commitée)** : migration `20260925120000_quote_lot_confirmations.sql` (registre `quote_lot_confirmations` append-only, empreinte de ligne métier, `read_lot_confirmation_context`, `record_lot_confirmation`, clôture composée `complete_lot_pricing` appelant le finaliseur PAD inchangé ; changement nommé de `record_pad_group_confirmation` : refus multi-lot remplacé par l'exigence d'une liaison fraîche, horodatage sous verrou) + rollback archivant sans perte ; `_shared/lot-confirmation(-store).ts` ; `pad-group-store.ts` (conditions par lot, allocation par ligne liée, instantané unique) ; Edge `manage-lot-confirmation` (+ config.toml) ; raccordement `run-pricing` (exception FROZEN bornée : lecture avant la boucle, mode terminal par lot, PAD par lot, contrôles D5 avant/après, clôture sous verrou, factorisation `confirmedPadLineInput`) ; UI `LotConfirmationsPanel`. Réconciliation de poids et commission CMA CGM par lot non ouvertes (lot bloqué, pas de formule nouvelle).
**Contrôles** : SQL PostgreSQL isolé (restauration schéma seul, réseau coupé) : registre, droits, CAS, idempotence, concurrence réelle deux sessions, rollback/relivraison PASS, suites PAD existantes PASS ; parcours local deux lots → calcul → lignes PAD par lot → clôture PASS ; Deno 2.9.5 complet 1712/1 (échec baseline identique) ; Vitest 554/555 (baseline) ; typecheck, lint baseline 732/16, typecheck Deno 49/5, build, config, bundles PASS. Contre-revue indépendante : passe 1 NO-GO (B1, double lecture du registre des lots), corrigé par un instantané unique ; remarques 1, 2, 4, 7 corrigées (horodatage sous verrou, resynchronisation du seul gap PAD, garde mono-lot, repère PAD nul) ; reprise limitée : GO avec réserves (remarques 3, 5, 6, 8 déclarées : bundle build-case-puzzle modifié transitivement, mode terminal porté par une ligne toujours accepté sans décision, perte de réponse après succès SQL comme en mono-lot, verrou `qrl_` distinct).
**Reste** : publication exécutée le 25/09 (entrée du 25/09 17:40 UTC) ; alerte non close tant que la recette déployée n'a pas réussi.
**Plan de publication (exécuté le 25/09)** : 1) commit/push sur work (Lovable synchronise l'interface ; le panneau des lots affiche « indisponible » tant que sa fonction n'existe pas) ; 2) migration `20260925120000` (voie Cloud à approuver) ; 3) déploiement de `manage-lot-confirmation`, `manage-pad-group-confirmation`, `run-pricing` ; 4) en dernier `build-case-puzzle` (seules ces quatre fonctions importent les modules modifiés) ; 5) recette authentifiée sur un sandbox à deux lots. Chaque état intermédiaire échoue de façon fermée : ancien build = gaps dossier conservés ; ancien run-pricing = lots bloqués. Rollback : redéployer les quatre fonctions depuis f060bea (retirer `manage-lot-confirmation`), puis script SQL avec accusé (registre archivé, jamais supprimé ni réactivé).
