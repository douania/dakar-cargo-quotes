...
| B1-B | Isolation case_documents + storage bucket | sécurité | deferred | Moyenne | M23c | 2026-03-28 | Flux upload-first/delete-first (CaseDocumentsTab.tsx) incompatible avec jointure RLS storage. Modèle shared workspace sur quote_cases non tranché pour documents. | Changement flux applicatif OU décision produit isolation documents | Storage `case-documents`, RLS `case_documents` | repo + chat B1 | Confirmé | Reconcevoir flux ou accepter shared workspace |
