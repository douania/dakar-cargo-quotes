...
| LOT_C2_PRE_NO_ALIAS_COVERAGE | Données | **🚫 DIFFÉRÉ — blocage data** | P2 | PAD_BAREME_2006_RUNTIME_EXPAND Lot C.2 | 2026-05-10 | Enrichissement des alias PAD ou arrivée d'un dossier conteneur avec cargo.description matchant un alias validé. | Shadow observation Lot C.2 bloqué : 0 dossier IMPORT/CONTENEUR croise un alias PAD validé (384 alias, 25 dossiers conteneur, 0 match). Le resolver et le shadow logging sont prêts (Lot C v3.1). Option A : enrichir alias depuis descriptions cargo réelles (mini-lot data + validation opérateur). Option B : attendre/fournir un dossier historique éligible. Ne pas activer `PAD_RESOLVER_SHADOW=true` avant déblocage. Voir `PAD_BAREME_2006_RUNTIME_EXPAND_LOT_C2_PRE_REPLAY_CANDIDATES.md`. |
