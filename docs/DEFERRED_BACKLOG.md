...
Dernière mise à jour : 2026-05-08 — **P1-C CLOS : guide des 5 conflits critiques NST→PAD créé (`PAD_NST_P1_C_CONFLICTS_GUIDE.md`). Doctrine opérateur pour ciment/clinker, phosphates, pétrole, plastiques, gaz naturel. P0-C CORRIGÉ : séquence C-B → C-D → C-B-LOG → C-E → C-C inscrite. DOC-R1 CLOS (commit 68e5e7c).**
...
| PAD-NST-2E-C-D | UI opérateur | **🚫 NO-GO temporaire** | P2 | PAD-NST-2E-C-B | 2026-05-08 | GO CTO séparé requis. Déblocage : C-B vérifié + GO CTO. | Affichage suggestions TO_CONFIRM + justification + confidence + source_document. Validation opérateur obligatoire. Interdit : auto-validation, OFFICIAL, amount automatique, écriture `pad_designation_aliases`. |
