...
| Champ | Valeur |
|-------|--------|
| **ID** | `CARRIER-CONTACT-CHANNELS-ADMIN-PANEL-1` |
| **Catégorie** | Admin / Maintenance référentiel carriers |
| **Statut** | `DEFERRED / IMPORTANT` |
| **Priorité** | P1 |
| **Phase d'origine** | CARRIER-CONTACT-CHANNELS-MIGRATION-1 / CARRIER-CONTACT-CHANNELS-SEED-1 / CARRIER-CONTACT-CHANNELS-UI-P1 |
| **Date** | 2026-06-07 |
| **Besoin** | Permettre à un admin autorisé de mettre à jour les emails, portails, contacts sales/service, `source_url`, `last_verified_at`, `service_types`, `is_active` et notes des carriers sans intervention développeur. |
| **Contexte** | Les contacts carriers changent dans le temps. Les canaux Maersk, Hapag-Lloyd, ONE/CSTT-AO, MSC et CMA CGM ont été seedés dans `known_business_contact_channels`, mais la maintenance future reste manuelle via DB/migration. |
| **Risque actuel** | Si un email carrier devient obsolète, l'app peut afficher un contact non à jour jusqu'à intervention technique. |
| **Exigences futures** | Écran admin sécurisé ; contrôle d'accès admin strict ; audit trail des changements ; validation des emails/URLs ; `source_url` obligatoire ou fortement recommandée ; `last_verified_at` mis à jour ; possibilité de désactiver un canal sans le supprimer ; pas d'écriture directe non auditée depuis le frontend. |
| **Hors périmètre immédiat** | Pas de modification RLS maintenant ; pas d'UI maintenant ; pas d'envoi email ; pas de pricing ; pas de changement sur les lots clos. |

Mise à jour : 2026-05-14 — **📋 MAPPING-TAX-CHAIN-0 → AUDIT V2 LIVRÉ (read-only) — `MAPPING_TAX_CHAIN_0_AUDIT_V2_READY`.** Refresh post-MAP-7B livré dans `docs/tariff-collection/MAPPING_TAX_CHAIN_0_AUDIT_V2.md` (V1 du 2026-05-09 conservé en historique, non modifié). **Q1** chaîne CN/NHM/NSTR → NST → PAD : **toujours NON automatique**, bridges (`nst_cn_mappings` 9 762, `nst_nhm_mappings` 15 079, `nstr_nst2007_mappings` 9 781, `nst_cpa_mappings` 1 759) restent dormants — aucun consommateur Edge / `src/` hors `types.ts` auto-généré. **Q2** PAD → taxe portuaire : `PORT_TAX` toujours 2 lignes TRANSIT only (aucun IMPORT ajouté), **mais découverte V2** : `port_tariffs` a doublé (98 → 218) et `DROIT_PASSAGE IMPORT` (38 lignes × 14 T-classes T01..T14) + `THD IMPORT` (19 × 14) sont désormais joignables sur `cargo.pad_category` — partie aval IMPORT activée pour DROIT_PASSAGE/THD. **Pivots runtime post-MAP-7B** : `cargo.hs_code` ✅ (175 facts, 15 current, consommé par run-pricing/set-case-fact/scope), `cargo.pad_category` ✅ (5 facts, 3 current, consommé par run-pricing/UI), `commodity.cn_code` / `commodity.nhm_code` / `commodity.nstr_code` / `commodity.nst_code` / `commodity.hs_code` / `pricing.pad_category` ❌ (0 fact, 0 consommateur, refusés par wrapper). **MAP-7B jamais déclenché en runtime client à ce jour** (0 fact `origin=MAP-7B`). Ambiguïtés mesurées : CN→NST 100 % unique, NHM→NST 100 % unique, NSTR→NST 52,6 % unique (47,4 % ambigus), NST→PAD 81,1 % unique. Pré-requis bloquants confirmés : `nst_groups` 73/81, divisions 15/20 absentes, ambiguïtés NSTR/NST, absence PORT_TAX IMPORT, absence pivot `nst_hs10_mappings`. Options A/B/C/D re-cadrées : A portée réduite, B partiellement réalisée, C acceptable si MAP-7B suffit, D dépend de la définition de « taxe portuaire ». Aucune décision d'exécution. **MAPPING-TAX-CHAIN-0 reste OUVERT** pour arbitrage CTO futur (chaque option = GO séparé). Aucun `src/`, aucune Edge Function, aucune migration, aucun GRANT, aucun `config.toml`, aucun `run-pricing`, aucun seed, aucun dossier client touché. MAP-6-EXEC-REST-AUTH-RECHECK / MAP-6-EXEC-UI / MAP-7B / MAP-6-AUTH-TEST-USERS-CLEANUP non modifiés.


### MAP-6-EXEC-REST-AUTH-RECHECK — ✅ CLOS (`MAP_6_EXEC_REST_AUTH_RECHECK_DONE_ACCEPTED_BY_EVIDENCE_CHAIN`)
- Catégorie : sécurité / tests E2E
- Statut : **done** — Priorité : closed — Phase origine : MAP-6-EXEC-MIGRATION — Date clôture : 2026-05-14
- Verdict CTO : `MAP_6_EXEC_REST_AUTH_RECHECK_DONE_ACCEPTED_BY_EVIDENCE_CHAIN` — B PASS, C PASS, E couvert par C, G couvert par H + MAP-6-SECURITY-GRANTS-FIX. Aucun seed, aucune écriture DB, aucun run-pricing, aucun code/GRANT modifié, aucun JWT affiché.
...
Mise à jour antérieure : 2026-05-14 — **✅ MAP-6-EXEC-MIGRATION (étape 1/2 — wrapper créé)** : migration DB-only appliquée, `public.propagate_classification_candidate_to_fact(uuid, text)` créée (`SECURITY DEFINER`, `SET search_path=public`), `REVOKE ALL ... FROM PUBLIC` + `GRANT EXECUTE ... TO authenticated`. Tests T1-T14 **non exécutés** : bloqué par `MAP_6_EXEC_BLOCKED_SECURITY_GRANT` (cf. ci-dessus).

Mise à jour antérieure : 2026-05-14 — **📋 MAP-6-RPC-WRAPPER-DESIGN livré (design-only) : Option C retenue suite à `MAP_6_EXEC_BLOCKED_RPC_PERMISSION` (RP3 KO — `authenticated` n'a pas EXECUTE sur `supersede_fact`). Wrapper RPC dédié `public.propagate_classification_candidate_to_fact(p_candidate_id, p_idempotency_key) RETURNS jsonb` (SECURITY DEFINER), borné métier : `case_id` dérivé du candidat (jamais paramètre), lock + `FOR UPDATE`, `has_case_write_access` après chargement, whitelist incluant `hs10_uemoa`, replay Niveau A (clé exigée) + Niveau B (sans filtre `is_current`) + détection `idempotency_conflict`, appel interne `supersede_fact` via `SELECT ... INTO`, écriture atomique `candidate.evidence` + `case_timeline_events` (`action_code='CCC_PROPAGATED_TO_FACTS'`), retours `jsonb` typés (pas de `RAISE` métier). **Aucun GRANT direct sur `supersede_fact`** — seul le wrapper sera exposé. Patch ciblé §3.5/§3.6/§3.10/§3.11 + tests T12 négatif `supersede_fact` non-grantée + T13 matrice auth/grants en 3 sous-cas (T13a EF sans JWT→401, T13b anon PostgREST→permission denied, T13c authenticated non-owner→`rls_write_denied`). Verdict : `MAP_6_RPC_WRAPPER_DESIGN_READY`. MAPPING-TAX-CHAIN-0 reste ouvert.**

Mise à jour antérieure : 2026-05-14 — **📋 MAP-6-DESIGN livré (design-only) : action explicite séparée "Propager au dossier" pour candidat `accepted, is_current=true`. Nouvelle Edge Function dédiée `propagate-classification-candidate-to-facts` (auth caller JWT, no service_role), appel direct `supersede_fact` avec `p_source_type='manual_input'`, `value_json` portant `candidate_id`+`propagation_idempotency_key` pour replay/recovery guard (palliatif absence de transaction Edge globale). Whitelist pivots MAP : `commodity.cn_code`, `commodity.hs_code`, `commodity.nhm_code`, `commodity.nst_code`, `commodity.nstr_code`, `pricing.pad_category` — distincts des facts consommés par `run-pricing` actuel, aucun effet pricing immédiat garanti. Précheck RPC permission `authenticated` EXECUTE bloquant pré-EXEC. Aucun rollback auto, aucun run-pricing, aucune modification `update-commodity-classification-candidate`/`set-case-fact`. Verdict : `MAP_6_DESIGN_READY`. MAPPING-TAX-CHAIN-0 reste ouvert.**

Mise à jour antérieure : 2026-05-14 — **✅ MAP-5B-V10-DEFERRED CLOS — test E2E auth V10 exécuté (auto-confirm temporaire, user test, appel EF, 403 `rls_write_denied` confirmé, seed rollback, Auth restauré, suppression manuelle UI Cloud des 3 users). Verdict CTO : `MAP_5B_V10_VALIDATED_403_RLS_WRITE_DENIED_TEST_USER_PENDING_MANUAL_DELETE_ACCEPTED`. MAP-5B-GLOBAL : `OPERATOR_ACTIONS_DEPLOYED_VALIDATED_ACCEPTED`. Aucune DB, aucun code, aucun runtime modifié.**
...
### MAP-6-RPC-WRAPPER-DESIGN — Wrapper RPC dédié `propagate_classification_candidate_to_fact` (Option C)

| Champ | Valeur |
|-------|--------|
| **ID** | `MAP-6-RPC-WRAPPER-DESIGN` |
| **Catégorie** | tariff-collection / sécurité RPC — wrapper borné métier exposé à `authenticated` |
| **Statut** | `📋 design-ready` |
| **Priorité** | Haute (bloque `MAP-6-EXEC-MIGRATION`) |
| **Phase d'origine** | MAP-6 (post précheck RP3 KO) |
| **Date** | 2026-05-14 |
...
### [Header historique] RFQ 451ab687 — export FCL Dakar→Djibouti (2026-06-07)

Mise à jour : 2026-06-07 — **✅ RFQ 451ab687 — export maritime FCL Dakar → Djibouti — corrections orchestrateur et facts clôturées.** Lots clos : `FACT-EXTRACTION-FCL-CONSTRAINTS-P0` **CLOS / PASS_RUNTIME_DEPLOY** commit `1cb422be` (20 MT per container n'est plus un poids total ; `cargo.weight_per_container_kg=20000`, `cargo.weight_kg=40000`, `pricing.destination_free_time_days=30`, `routing.final_destination=Ethiopia`, `routing.transit_via_port=Djibouti Sea Port`) ; `CASEVIEW-REQUESTTYPE-PACKAGE-LABEL-P1` **CLOS / PASS_RUNTIME_VISUAL** commit `d08d0d7f` (double badge CaseView : Mode technique `SEA_FCL_IMPORT`, Package métier `EXPORT SENEGAL`, sans migration enum `quote_request_type`) ; `MULTI-HS-RFQ-SUGGESTION-P1` **CLOS / PASS_RUNTIME_WITH_NOTES** commit `f3b62f88` (HS source `84311000` conservé en suggestions HS10/timeline, valeurs HS combinées non tronquées, `cargo.hs_code` reste un HS10 canonique unique) ; `EXPORT-FCL-SEA-FREIGHT-ORCHESTRATION-P0` **CLOS / PASS_RUNTIME_DEPLOY** commit `5437e3d` (`EXPORT_SENEGAL` + `SEA_FREIGHT` sans couverture maritime crée le gap bloquant `pricing.sea_freight_partner_quote_required` + `external_quote_request` draft `purpose=freight_rate`, et fait repasser 451ab687 de `READY_TO_PRICE` à `NEED_INFO`) ; `EXPORT-FCL-PARTNER-RFQ-SEND-SMOKE-1` **CLOS / PASS_WITH_NOTES** (demande partenaire `freight_rate` draft présente, aucun email réel envoyé, aucun pricing lancé, `purpose_detail` contient route, destination finale, incoterm, conteneurs, poids, free time, package et service `SEA_FREIGHT`). Règles canoniques : un export maritime FCL `service.package=EXPORT_SENEGAL` avec `SEA_FREIGHT` ne doit pas être prêt à chiffrer sans couverture maritime partenaire ou demande `freight_rate` ouverte ; `cargo.hs_code` reste un fact canonique unique et les sources multi-HS restent suggestions/timeline ; `quote_cases.request_type` peut rester le mode technique, le package métier est porté par `service.package`. Dettes restantes : armateurs connus sans `contact_email`, `service_types` vide sur certains armateurs, pas de preview email partenaire avant clic Envoyer, template `freight_rate` dépend fortement de `purpose_detail`, `request_type=SEA_FCL_IMPORT` reste structurellement incohérent pour les exports mais mitigé par le double badge. Tests ciblés Deno passés pour FCL constraints, Multi-HS et Export SEA_FREIGHT orchestration ; runtime smoke 451ab687 validé ; aucun pricing lancé pendant ces chantiers.

---

### [Header historique] PAD-V5-SHADOW-DOC-UPDATE-1 (2026-06-02)

Mise à jour : 2026-06-02 — **✅ PAD-V5-SHADOW-DOC-UPDATE-1 PASS / DONE.** Le chantier PAD V5 shadow import est clos avec succès : table `public.pad_cn2008_mapping_v5_shadow` créée et importée en **shadow only / QA**, source `V5_2026-06-02`, hash `ae758ca9cf5dfc7b1e3c29f56e8e8b664086b0d6bdfce5a31f138f9deddb88d8`, total `9 777` lignes (`AUTO_SAFE=696`, `AUTO_SAFE_CANDIDATE=716`, `TO_CONFIRM=8 332`, `DORMANT=33`, `REJECT=0`). A1 PASS (table vide avec RLS, policy SELECT authenticated, 7 index), A1B PASS (`cpa2008_code`, `cpa2008_label`, `row_key` documenté), A2-v2 PASS (import `mapping_v5.csv`). Non-contamination confirmée : `port_tariffs`, `commodity_classification_candidates`, `operator_decisions`, `pad_recommendation_audit_log`, `pad_nst_recommendation_rules`, `pad_designation_aliases` inchangés. Aucun runtime, aucun `run-pricing`, aucune Edge Function, aucun calcul `PAD_DROIT_PASSAGE` depuis V5. **Frontière prix : `port_tariffs / PAD / DROIT_PASSAGE` reste l’unique source de prix ; V5 ne calcule jamais `PAD_DROIT_PASSAGE`.** `AUTO_SAFE_CANDIDATE` requiert validation opérateur et ne doit jamais être auto-promu. Phases suivantes **DEFERRED / NO-GO** tant que non rouvertes : `PAD-V5-PHASE-B-OPERATOR-UI` (uniquement après validation CTO d’un plan UI/operator workflow), `PAD-V5-PHASE-C-SHADOW-RUNTIME` (uniquement après Phase B PASS), `PAD-V5-PHASE-D-AUTOSAFE-FEATURE-FLAG` (uniquement après shadow comparison sur cas réels + feature flag + tests), `PAD-V5-PHASE-E-CANDIDATE-VALIDATION` (jamais sans opérateur-in-the-loop).

---

### [Header historique] MAP-8B (2026-05-14)

Mise à jour : 2026-05-14 — **✅ MAP-8B CLOS (`MAP_8B_EXEC_MIGRATION_DONE`).** Patch chirurgical DB-only du wrapper `public.propagate_classification_candidate_to_fact(uuid,text)` : la branche `pad_category` matérialise désormais `cargo.pad_rate_fcfa_per_ton` depuis `port_tariffs` (provider=PAD, category=DROIT_PASSAGE, operation_type=IMPORT, cargo_type=CONTENEUR, classification=v_value, is_active=true) en plus de `cargo.pad_category`. Garde scope obligatoire : `cargo.containers` current non vide sinon `unsupported_pad_rate_scope`. Tarif unique exigé sinon `pad_rate_not_found` / `pad_rate_ambiguous`. Réutilisation rate fact si `value_number` identique (`already_current_same_amount`). Replay strict avec `is_current=true` sur `cargo.pad_category`. Bypass des early-returns Niveau A/B uniquement pour `pad_category` afin de garantir les 2 facts en sortie. Variables scalaires `uuid`/`numeric` (pas de `record`). Garde-fou timeline : aucun `case_timeline_events` inséré lors d'un replay complet. Snapshot live pré-migration conservé (`docs/tariff-collection/pad/MAP_8B_FUNCTION_SNAPSHOT_PRE.sql`). Préchecks PASS : grants matrix inchangée, 0 doublon actif PAD/DROIT_PASSAGE/IMPORT/CONTENEUR, T12=4780 unique. Tests sandbox T1–T7 PASS en transaction avec ROLLBACK : T1 (`created` value=4780), T2 (`replayed_idempotency`, 0 nouveau fact/timeline), T3 (`invalid_pad_category`), T4 (`pad_rate_not_found`), T4bis (`unsupported_pad_rate_scope`), T4ter (`already_current_same_amount`, 0 supersession rate), T5 (`hs10_uemoa` inchangé, pas de `rate_fact_id`), T6 (`pad_label_forbidden`), T7 (grants identiques). Postchecks case_a : 1 `cargo.pad_category` + 1 `cargo.pad_rate_fcfa_per_ton`=4780 current, 2 timeline events pour 4 calls (garde-fou OK), 0 `pricing_runs`, 0 résidu sandbox. Limitation : **flux import conteneur standard uniquement**. Branches `hs10_uemoa` / `pad_label` / `cn8` / `hs6` / `nhm` / `nst2007` / `nstr` inchangées au caractère près. Aucun `src/`, aucune Edge Function, aucun `config.toml`, aucun GRANT, aucune modification de `public.supersede_fact`, aucun `run-pricing`, aucun seed permanent, aucun dossier client réel touché. **MAPPING-TAX-CHAIN-0 reste OUVERT**.

---

### [Header historique] MAP-8 Audit (2026-05-14)

Mise à jour : 2026-05-14 — **📋 MAP-8 — AUDIT RUNTIME `cargo.pad_category` → DROIT_PASSAGE / THD / PORT_TAX LIVRÉ (read-only) — `MAP_8_RUNTIME_PAD_CATEGORY_TO_PORT_CHARGES_AUDIT_READY`.** `cargo.pad_category` est lu par `run-pricing` (L2913) mais ne suffit pas seul : l'émission `PAD_DROIT_PASSAGE` (L2200) exige `padRateFcfaPerTon > 0`, donc 3 facts sont requis (`cargo.pad_category` + `cargo.pad_rate_fcfa_per_ton` + `cargoWeight`). Les 3 dossiers actuels avec `cargo.pad_category` (T07, T12, T01) ont tous `cargo.pad_rate_fcfa_per_ton` (HAS_RATE_FACT). `DROIT_PASSAGE IMPORT` existe (38 lignes) et matche T01/T07/T12 sur classification brute. `THD IMPORT` existe (19 lignes) mais classification = texte long (« T01 - Drinks… ») → ne matche pas `cargo.pad_category` brut (« T01 »). `PORT_TAX IMPORT` = 0 ligne (toujours 2 lignes TRANSIT only). THD est une chaîne carrier/description séparée (HAPAG_LLOYD), pas `cargo.pad_category`. **Risque principal** : MAP-7B propage `cargo.pad_category` mais pas automatiquement `cargo.pad_rate_fcfa_per_ton` — sans ce 2nd fact, `PAD_DROIT_PASSAGE` n'est pas émis. **Recommandation** : ouvrir MAP-8B en plan séparé pour matérialiser `cargo.pad_rate_fcfa_per_ton` depuis `port_tariffs` `DROIT_PASSAGE IMPORT CONTENEUR` lors de la propagation `pad_category`. **MAPPING-TAX-CHAIN-0 reste OUVERT**. Aucune décision Option A/B/C/D. Aucun `src/`, aucune Edge Function, aucune migration, aucun GRANT, aucun `config.toml`, aucun `run-pricing`, aucune écriture DB, aucun seed, aucun dossier client touché.

---

### [Header historique] MAP-7B (2026-05-14)

Mise à jour : 2026-05-14 — **✅ MAP-7B CLOS (`MAP_7B_EXEC_MIGRATION_DONE_ACCEPTED`).** Migration appliquée : remplacement du wrapper `public.propagate_classification_candidate_to_fact(uuid,text)` avec mapping canonique runtime (`pad_category` → `cargo.pad_category`, `hs10_uemoa` → `cargo.hs_code`) et garde regex `^[TPC][0-9]{2}$` sur `pad_category`. Snapshot pré-MAP-7B conservé (`docs/tariff-collection/pad/MAP_7B_FUNCTION_SNAPSHOT_PRE.sql`). Tests sandbox T1–T10 passés (simulation `auth.uid` via `set_config`, rollback granulaire, 0 résidu, 0 `pricing_runs`). `hs6` / `cn8` / `nhm` / `nst2007` / `nstr` restent refusés (`candidate_kind_not_whitelisted`) — différés vers `MAPPING-TAX-CHAIN-0`. Grants inchangés (`wrapper_authenticated=t`, `wrapper_anon=f`, `wrapper_service_role=t`, `supersede_authenticated=f`, `supersede_anon=f`, `supersede_service_role=t`). Aucun dossier client réel touché. Aucun `src/`, aucune Edge Function, aucun `config.toml` modifié. **Note historique** : à la clôture de MAP-7B, MAP-6-EXEC-UI était encore bloquée ; cette dette a été levée ultérieurement par l'arbitrage CTO Option 1 (`MAP_6_EXEC_REST_AUTH_RECHECK_DONE_ACCEPTED_BY_EVIDENCE_CHAIN`) puis par le lot `MAP_6_EXEC_UI_DONE_ACCEPTED_AND_ROLLED_BACK` documenté ci-dessous. MAP-7B n'a pas, à lui seul, débloqué l'UI. MAPPING-TAX-CHAIN-0 reste ouvert.

---

### [Header historique] MAP-6-EXEC-UI (2026-05-14)

Mise à jour antérieure : 2026-05-14 — **✅ MAP-6-EXEC-UI DONE (`MAP_6_EXEC_UI_DONE_ACCEPTED_AND_ROLLED_BACK`).** Bouton « Propager au dossier » exposé uniquement sur candidat `accepted + is_current=true + non-propagé`. Dialog de confirmation validé (annulation = aucun appel réseau). Confirmation = un seul `POST /functions/v1/propagate-classification-candidate-to-facts` avec body strict `{candidate_id, idempotency_key}` (pas de `case_id`). Aucun appel direct RPC (`propagate_classification_candidate_to_fact` / `supersede_fact`) depuis le frontend. Aucun `run-pricing` déclenché. Succès serveur confirmé end-to-end (candidate.evidence.propagated_fact_id + quote_fact + case_timeline_event `CCC_PROPAGATED_TO_FACTS` cohérents). Idempotency-key namespacée `propagate:${candidate_id}`, conservée sur erreurs réseau/5xx, purgée sur succès et erreurs métier fatales. Mapping erreurs 400/403/404/409/422/500 → toasts dédiés. Code review CTO accepté, runtime validé via lot seed contrôlé `MAP-6-EXEC-UI-RUNTIME-SEED` puis rollback intégral. Un seul fichier modifié sur ce lot : `src/components/case/CommodityClassificationCandidatesPanel.tsx`. **MAP-6-EXEC-UI-RUNTIME-SEED → ROLLED_BACK / CLOSED** : 1 timeline event, 1 quote_fact (supersedes_fact_id NULL → pas de réactivation), 1 candidate seed supprimés par IDs explicites + filet `seed_lot='MAP-6-EXEC-UI-RUNTIME-SEED'`. Postcheck final : candidates_seed=0, facts_seed=0, timeline_seed=0, pricing_runs_test=0. Aucun nouveau user Auth, aucun JWT forgé, aucune migration, aucun GRANT, aucun code modifié pendant le rollback.

---

### [Header historique] MAP-6-SECURITY-GRANTS-FIX (2026-05-14)

Dernière mise à jour : 2026-05-14 — **✅ MAP-6-SECURITY-GRANTS-FIX DONE (Option S1 — révocation ciblée).** Migration DB-only appliquée : `REVOKE EXECUTE ON public.supersede_fact(...) FROM PUBLIC, anon, authenticated;` + `REVOKE EXECUTE ON public.propagate_classification_candidate_to_fact(uuid,text) FROM PUBLIC, anon;` + `GRANT EXECUTE ON public.propagate_classification_candidate_to_fact(uuid,text) TO authenticated;`. Validation `has_function_privilege` 6/6 conformes (`wrapper_authenticated=t`, `wrapper_anon=f`, `wrapper_service_role=t`, `supersede_authenticated=f`, `supersede_anon=f`, `supersede_service_role=t`). Validation PostgREST réelle via user Auth éphémère (signUp + signInWithPassword) : **P1** anon `/rpc/supersede_fact` → 401 `permission denied for function supersede_fact` ; **P2** authenticated `/rpc/supersede_fact` → 403 `permission denied for function supersede_fact` ; **P3** anon `/rpc/propagate_classification_candidate_to_fact` → 401 `permission denied for function` ; **P4** authenticated `/rpc/propagate_classification_candidate_to_fact` → 200 `{ "ok": false, "code": "candidate_not_found" }` (callable, réponse métier wrapper). EF appelantes (`build-case-puzzle`, `set-case-fact`, `validate-partner-fact`) inchangées — toutes utilisent `serviceClient` (`SUPABASE_SERVICE_ROLE_KEY`), `service_role` conservé. **Résidus connus** : 2 users Auth de test non supprimables via psql (schéma `auth` interdit) — `dd45375a-aae3-4791-9d0e-add174b29668` + `e3999a32-8aec-4318-bef0-6c2a9453d8e3` (`map6-sec-test-…@example.test`) → suppression manuelle UI Cloud requise. `auto_confirm_email` toggle temporaire ON puis remis à OFF immédiatement après. **Verdict : `MAP_6_SECURITY_GRANTS_FIX_DONE`**. Réouverture de `MAP-6-EXEC-MIGRATION` autorisée pour T1-T14 sur GO CTO séparé.

---

### [Header historique] MAP-6-EXEC-MIGRATION T1–T12 (2026-05-14)
