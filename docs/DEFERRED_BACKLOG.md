...
| DT-NOMENCLATURE | Peuplement `commodity_categories` avec codes terminal Dakar Terminal (storage uniquement) | donnée | deferred | Moyenne | DT-V1 | 2026-04-02 | Peuplement partiel conservateur effectué : 3/10 catégories PAD peuplées (T05→410, T09→414, T14→412). 7 catégories laissées NULL (trop hétérogènes). Traçabilité via `notes_operator`. | Mapping fin par désignation ou sous-classe pour les 7 catégories ambiguës (T01, T02, T03, T04, T07, T12, T13) | `commodity_categories` colonnes `terminal_storage_code_p1/p2/p3` | repo + grille officielle 2014 + factures TOM | Confirmé | Périmètre strictement magasinage / codification terminal. Ne pas étendre à la manutention ; DPW reste le référentiel séparé via `port_tariffs`. |
| DT-FINE-MAPPING | Mapping fin désignation → code terminal pour catégories PAD ambiguës | architecture | deferred | Moyenne | DT-V2 | 2026-04-02 | Les catégories T01, T02, T03, T04, T07, T12, T13 sont trop hétérogènes pour un mapping "1 code dominant". Le bon modèle futur serait un mapping par désignation ou sous-classe métier, potentiellement via une table dédiée plutôt que dans `commodity_categories`. | Besoin moteur de calculer le magasinage réel pour ces catégories | `commodity_categories`, potentiellement nouvelle table de mapping | grille officielle 2014 | Confirmé | Ne pas forcer un mapping grossier dans `commodity_categories`. Concevoir un modèle plus fin quand le besoin moteur se matérialise. |
| DT-RATE-TABLE | Table `terminal_tariff_codes` — peuplement initial storage injecté (28 lignes), handling et révision 419 à suivre | architecture | deferred | Moyenne | DT-V1 | 2026-04-02 | Table créée + peuplement initial storage Dakar Terminal 2014 : 28 lignes (10 familles, P1/P2/P3), 27 `official`, 1 `to_confirm` (419/P1). Notes corroboration sur 519/619. Famille 420 : P1 seul (P2/P3 non documentés). **Handling non injecté. DPW non injecté.** | Prochaine étape : investigation écart 419, puis éventuel peuplement handling ou intégration moteur | `public.terminal_tariff_codes` | grille officielle 2014 + factures TOM | Confirmé | Périmètre strictement magasinage / codification terminal. Ne pas étendre à la manutention ; DPW reste le référentiel séparé via `port_tariffs`. |
