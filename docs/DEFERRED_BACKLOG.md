...
Dernière mise à jour : 2026-05-02 — **LOT3-A exécuté** : quarantaine P0 minimale + filtres runtime essentiels. (1) `price-service-lines` : filtre `.eq("status", "active")` ajouté sur `pricing_rate_cards` — 35 lignes `to_confirm` exclues (0 active). (2) `quotation-engine` : filtre `.in("evidence_level", ["official", "validated_internal"])` ajouté sur `demurrage_tiers` — 2 paliers `observed` MSC exclus. (3) Data quarantine : 10 `destination_terminal_rates` Taleb + 6 `border_clearing_rates` Taleb + 9 `demurrage_rates` non vérifiées (COSCO/Evergreen/ONE) → `is_active=false`. Total P0 neutralisé : 62 lignes. LOT3-0 clos. LOT2-REV-B/C inchangés. INFRA-PUBLISH inchangé.
