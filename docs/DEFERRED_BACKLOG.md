...
| LEG-CLEANUP | Retrait fonctions legacy pures (create-quotation-draft, generate-quotation, generate-quotation-pdf) | dette | deferred | Basse | LEG-audit | 2026-03-29 | Ces 3 edge functions n'ont d'appelant que dans QuotationSheet (désormais verrouillé quand case existe). Pas de risque métier tant que le verrouillage UI est en place. Retrait possible en phase de nettoyage ultérieure. | QuotationSheet `/quotation/new` supprimé, ou décision de retirer le legacy complètement | `create-quotation-draft`, `generate-quotation`, `generate-quotation-pdf` | repo + audit pipeline | Confirmé | Garder tant que `/quotation/new` est un point d'entrée valide |
