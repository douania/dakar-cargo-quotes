...
Dernière mise à jour : 2026-05-02 — **LOT3-A-VALIDATION clos analytique**. Les sources P0 identifiées dans LOT3-0 et traitées par LOT3-A ne peuvent plus sortir automatiquement en cotation client (validation analytique, pas smoke runtime). LOT3-0 clos. LOT3-A clos. Synthèse tarifaire globale produite : `docs/SYNTHESE_TARIFAIRE_POST_NETTOYAGE.md`. Point découvert : `run-pricing` PAD alias sans filtre `evidence_level` (4 lignes `observed` PAD potentiellement consommables — LOT3-B-PAD). LOT2-REV-B/C inchangés. INFRA-PUBLISH inchangé.
