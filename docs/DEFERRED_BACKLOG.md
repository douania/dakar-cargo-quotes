...
| DT-RATE-TABLE | Table `terminal_tariff_codes` — structure créée, peuplement à suivre | architecture | deferred | Moyenne | DT-V1 | 2026-04-02 | Table créée (migration réussie) avec contraintes : `tariff_type` IN (storage, handling), `terminal_provider` IN (dakar_terminal, dpw), cohérence tariff_type/period, amount ≥ 0, UNIQUE(code, provider, period), index lookup. **0 donnée injectée.** Peuplement différé : montants 2014 à consolider avant injection (écart P1 observé). | Décision sur la stratégie de source/version/révision des montants, puis peuplement effectif | `public.terminal_tariff_codes` | grille officielle 2014 | Confirmé | Périmètre strictement magasinage / codification terminal. Ne pas étendre à la manutention ; DPW reste le référentiel séparé via `port_tariffs`. Prochaine étape : peuplement progressif après consolidation des montants actuels |
