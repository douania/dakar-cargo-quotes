...
| DT-NOMENCLATURE | Peuplement `commodity_categories` avec codes terminal Dakar Terminal (handling + storage) | donnée | deferred | Moyenne | DT-V1 | 2026-04-02 | Grille Dakar Terminal 2014 analysée comme référence de structure/nomenclature. ~500 désignations avec codes manutention (101-149) et magasinage (4xx/5xx/6xx). Codes validés par liaison factures TOM (position 138, codes 419/519/619). Montants pas encore pleinement consolidés (écart P1 observé à investiguer). | Décision produit d'enrichir le matching désignation → terminal, ou besoin moteur de calculer le magasinage par poids | `commodity_categories` colonnes `terminal_handling_code`, `terminal_storage_code_p1/p2/p3` | repo + grille officielle 2014 + factures TOM | Confirmé | Peupler progressivement en commençant par les catégories PAD existantes (T01-T14), puis étendre |
| DT-RATE-TABLE | Création table `terminal_tariff_codes` (codes → montants FCFA) | architecture | deferred | Moyenne | DT-V1 | 2026-04-02 | La grille Dakar Terminal 2014 contient 2 tables de conversion code → montant FCFA (manutention page 33, magasinage page 34). Ces tables sont nécessaires pour résoudre les codes stockés dans `commodity_categories` en montants calculables. Aucune table équivalente n'existe en base. | Besoin moteur de calculer le magasinage terminal par poids × jours × taux, ou injection de données terminales dans le pricing | Nouvelle table `terminal_tariff_codes` | grille officielle 2014 | Confirmé | Créer la table avec : code, montant FCFA, type (manutention/magasinage), période (P1/P2/P3 pour magasinage), unité, source_document, evidence_level |
| DT-2014-REVISION | Investigation écart taux P1 magasinage Dakar Terminal (grille 2014 vs facture TOM) | donnée | pending_validation | Basse | DT-V1 | 2026-04-02 | Écart observé à investiguer : code 419 (P1 magasinage) = 1 768 FCFA/T/j dans grille 2014 vs 1 964 FCFA/T/j sur facture TOM récente (+11%). P2 et P3 exacts. Cause possible : révision partielle, contexte TOM/TCD spécifique, arrondi, ou surcharge locale. | Obtention d'une grille Dakar Terminal post-2014, ou d'une deuxième facture TOM avec même code pour comparer | Grille 2014 page 34 (code 419), facture TOM (position 138) | grille officielle + facture réelle | Confirmé | Ne pas utiliser le montant 2014 du code 419 comme vérité tarifaire sans consolidation. Les montants P2 (519) et P3 (619) sont exploitables tels quels |
