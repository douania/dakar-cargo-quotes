...
| DT-NOMENCLATURE | Peuplement `commodity_categories` avec codes terminal Dakar Terminal (storage uniquement) | donnée | deferred | Moyenne | DT-V1 | 2026-04-02 | Grille Dakar Terminal 2014 analysée comme référence de structure/nomenclature. ~500 désignations avec codes magasinage (4xx/5xx/6xx). Codes validés par liaison factures TOM (position 138, codes 419/519/619). Montants pas encore pleinement consolidés (écart P1 observé à investiguer). | Décision produit d'enrichir le matching désignation → terminal, ou besoin moteur de calculer le magasinage par poids | `commodity_categories` colonnes `terminal_storage_code_p1/p2/p3` | repo + grille officielle 2014 + factures TOM | Confirmé | Périmètre strictement magasinage / codification terminal. Ne pas étendre à la manutention ; DPW reste le référentiel séparé via `port_tariffs`. Peupler progressivement en commençant par les catégories PAD existantes (T01-T14) |
| DT-RATE-TABLE | Création table `terminal_tariff_codes` (codes magasinage → montants FCFA) | architecture | deferred | Moyenne | DT-V1 | 2026-04-02 | La grille Dakar Terminal 2014 contient une table de conversion code magasinage → montant FCFA (page 34). Ces tables sont nécessaires pour résoudre les codes stockés dans `commodity_categories` en montants calculables. Aucune table équivalente n'existe en base. | Besoin moteur de calculer le magasinage terminal par poids × jours × taux, ou injection de données terminales dans le pricing | Nouvelle table `terminal_tariff_codes` | grille officielle 2014 | Confirmé | Périmètre strictement magasinage / codification terminal. Ne pas étendre à la manutention ; DPW reste le référentiel séparé via `port_tariffs`. Créer la table avec : code, montant FCFA, type magasinage, période (P1/P2/P3), unité, source_document, evidence_level |
