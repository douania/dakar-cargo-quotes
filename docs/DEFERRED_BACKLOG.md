...
## Railway — Sujets différés post DCQ-RAILWAY-BOUNDARY-AUDIT

| ID | Catégorie | Statut | Priorité | Phase d'origine | Date | Déclencheur de réouverture | Recommandation |
|----|-----------|--------|----------|-----------------|------|---------------------------|----------------|
| DCQ-RAILWAY-INTAKE-MIGRATION | Architecture / Edge Functions | **🟡 AUDIT-DONE / AWAITING PHASE 1** | P2 | DCQ-RAILWAY-BOUNDARY-AUDIT | 2026-05-13 | GO CTO pour Phase 1 (feature flag `VITE_INTAKE_BACKEND=railway\|edge` dans `Intake.tsx`, default `railway`) puis Phase 2 (création Edge Function `intake-case-request`). | Phase 2 : Edge Function `intake-case-request` réutilisant `parse-document` + `ensure-quote-case` (mode intake adapté pour générer `case_id` côté edge) + `build-case-puzzle` + `set-case-fact`. Recréer la classification complexité (`complexity_level`, `workflow_key`, `confidence`, `assumptions`) via Lovable AI Gateway (`google/gemini-2.5-flash` ou `openai/gpt-5-mini`), schéma `IntakeResponse` strictement identique à Railway pour drop-in compatibilité (UI Intake L315+ mappe `missing_fields[].field`). Phase 3 : flip flag par défaut `edge`, observation 1–2 semaines. Phase 4 : suppression `railwayApi.ts` (conditionnée à audit truck clos). **Tests obligatoires avant Phase 3** : parité Railway vs Edge sur ≥10 cas réels (snapshot `IntakeResponse`), matrice (texte libre court/long, email forwardé, WhatsApp brut, PDF, image), respect whitelist `ALLOWED_REQUEST_TYPES` de `ensure-quote-case`. **Interdictions** : aucune modif `run-pricing`, `quotation-engine`, PAD-NST, truck loading. Voir `docs/audits/DCQ-RAILWAY-BOUNDARY-AUDIT.md`. |
| DCQ-RAILWAY-TRUCK-LOADING-AUDIT | Architecture / Truck loading | **🟡 TODO — audit séparé** | P3 | DCQ-RAILWAY-BOUNDARY-AUDIT | 2026-05-13 | GO CTO pour audit dédié. | Auditer l'incohérence : `src/services/truckLoadingService.ts` appelle Railway en direct sur 4 endpoints (`/api/optimization/truck-specs` L367, `/optimize` L446, `/visualize` L490, `/suggest-fleet` L622) **et** invoque `truck-optimization-proxy` Edge Function (L279) qui expose les mêmes 4 endpoints. Décider : (A) tout passer par le proxy edge (avec auth Bearer JWT, déjà en place) et supprimer les 4 `fetch` directs, OU (B) supprimer le proxy et garder les `fetch` directs avec auth ajoutée. Conserver ou non la dépendance Railway pour l'optimisation 3D (algorithme génétique, pas trivial à migrer). **Hors scope migration intake.** |
| DCQ-RAILWAY-DEAD-EXPORTS | Code hygiene / `src/services/railwayApi.ts` | **🟡 DEPRECATE-PENDING** | P3 | DCQ-RAILWAY-BOUNDARY-AUDIT | 2026-05-13 | Sous-lot `@deprecated` JSDoc immédiat (Phase 0), suppression Phase 4 après migration intake. | `fetchCaseFile()` (L114) et `runWorkflow()` (L129) n'ont **aucun consommateur** dans `src/` (vérifié `rg`). Action immédiate : ajouter `/** @deprecated dead export — no consumer in src/, scheduled removal Phase 4 */` au-dessus de chaque fonction. Suppression complète seulement après suppression de `railwayApi.ts` (Phase 4, conditionnée à migration intake + décision truck loading). |
