...
| PAD-BAREME-2006-PHASE2-IMPORT | Données | **📋 AUDIT/PLAN LIVRÉ — 🚫 NO-GO exécution** | P1 | PAD-BAREME-2006 | 2026-05-10 | GO CTO séparé requis pour 2a (brouillon SQL documentaire), 2c (GO migration), 2d (application). Déblocage : validation CTO de `PAD_BAREME_2006_PHASE2_IMPORT_STRATEGY.md` (7 points §18). Interdit : migration appliquée, import 124/120 lignes, patch runtime, edge function, modification `src/`/CSV/manifest. | Stratégie d'import 120 lignes PRESENT (4 BLANK_IN_PDF exclus). Recommandations : `source_document='pdf_redevances_portuaires_2006'` (continuité legacy), R2 (désactivation 19 legacy + INSERT 120), index unique partiel sur `(provider,category,operation_type,classification,cargo_type) WHERE is_active=true`, `effective_date='2006-01-01'`. FK scan = 0 (vérifié). Aucun patch runtime requis (filtres IMPORT/CONTENEUR déjà actifs post RT-PREIMPORT-1). Voir `PAD_BAREME_2006_PHASE2_IMPORT_STRATEGY.md`. |
| PAD-BAREME-2006-RUNTIME-EXPAND | Runtime | **🚫 NO-GO — phase ultérieure** | P3 | PAD-BAREME-2006-PHASE2-IMPORT | 2026-05-10 | Activation runtime EXPORT / TRANSIT_IMPORT / TRANSIT_EXPORT / TRANSBORDEMENT et CONVENTIONNEL côté `run-pricing` / `recommend-pad-category`. GO CTO séparé requis. Déblocage : Phase 2 import exécutée + besoin métier exprimé + audit lookup-by-cargo. | Ne PAS confondre avec Phase 2 import (data-only). Cette phase concerne uniquement l'élargissement des filtres runtime au-delà d'`IMPORT/CONTENEUR`. Risque latent : tout lookup oubliant le filtre `cargo_type` retourne 2 lignes après Phase 2. |
