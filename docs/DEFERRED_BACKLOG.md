...
| LOT_C2_PRE_NO_ALIAS_COVERAGE | Données | **🟡 UNDER_ENRICHMENT_REVIEW — blocage data ouvert** | P2 | PAD_BAREME_2006_RUNTIME_EXPAND Lot C.2 | 2026-05-10 (MAJ 2026-05-11 par C.2-pre+1) | Validation opérateur du candidat `carreaux` → T12 (Lot C.2-pre+2 séparé) ET enrichissement des 24 familles `NO_PAD_MATCH_FOUND`. | Shadow observation Lot C.2 bloqué : 0 dossier IMPORT/CONTENEUR croise un alias PAD validé (384 alias, 25 dossiers conteneur, 0 match). C.2-pre+1 (read-only) a identifié **1 candidat propre `TO_VALIDATE_OPERATOR`** (`carreaux` → T12 via near match sur `carreaux ceramiques`/`carreaux en ceramique`) et **24 dossiers `NO_PAD_MATCH_FOUND`** (Energy storage, Steel tubes, Salt, Avions jouets, Vehicles, Wooden posts, Aluminium minerai, Cashew nuts, PET flakes, Pellets, Geomembranes, Used machine, Water equipment, sel, General cargo, Power generators, accumulator, Materiel electrique, rechange). 0 zone de conflit (ni listée ni multi-sources dynamique). Statut passé en `under_enrichment_review` car ≥ 1 candidat viable existe ; **non clôturé** : aucun alias n'est injecté avant Lot C.2-pre+2 (validation opérateur). Ne pas activer `PAD_RESOLVER_SHADOW=true` tant qu'aucun alias supplémentaire n'a été injecté. Voir `PAD_BAREME_2006_RUNTIME_EXPAND_LOT_C2_PRE_REPLAY_CANDIDATES.md` et `PAD_BAREME_2006_RUNTIME_EXPAND_LOT_C2_PRE_PLUS_1_ALIAS_ENRICHMENT_CANDIDATES.md`. |
