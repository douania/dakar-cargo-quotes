...
| HS10-AUTO-INJECTION-GUARD | Governance / Edge function `build-case-puzzle` (paths M3.4b, M3.4c email, Post-Attach) | **🟡 AUDIT-DONE — patch backend NO-GO sans GO CTO séparé** | P1 | HS10-RANKING-CONTEXT-ENRICHMENT (Test B) | 2026-05-12 | Constat Test B `31efcc01` : `cargo.hs_code` passe de `NULL` → `4403110000` avec `source_type=document_regex` sans validation opérateur. Contredit la doctrine `HS10-SUGGESTION-UI-SELECTOR` (« écriture `cargo.hs_code` réservée au clic opérateur »). 3 paths d'écriture auto inventoriés : M3.4b `[HS doc-regex]` (L3260-3273 mono, L3295-3308 multi-CSV), M3.4c `[HS email-regex]` (L3418-3431 mono, L3453-3466 multi-CSV), Post-Attach `[HS Resolution]` (L3991-4004). Sub-10 chiffres déjà conformes (suggestion only). | **Doctrine cible validée CTO = Option C (règle hybride conditionnelle)**. Critères cumulatifs pour autoriser écriture auto `cargo.hs_code` : (1) `sourceLen === 10` ; (2) `resolveSenegalHsCode === "unique"` ; (3) cohérence cross-source si plusieurs documents/emails contiennent un HS10 (tous résolvent au même `code10`) ; (4) **aucune divergence DD/TVA entre les candidats HS10 du même SH6** (à vérifier via query `SELECT DISTINCT dd, tva FROM hs_codes WHERE code_normalized LIKE '<sh6>%'`) ; (5) source explicitement labellisée `HS`/`HSCODE`/`Code Douanier` (déjà géré par `extractHsCodesFromTextDetailed`). Si ❌ un critère → suggestion `HS10_CLASSIFICATION_SUGGESTION` `status=trace` + GAP `cargo.hs_code` ouvert. Si ✅ tous → écriture auto + event forensic via `event_type='manual_action'` avec `event_data.action_code='HS10_AUTO_INJECTION'`, `event_data.status='trace'`, **sans `dedupe_key`** (le CHECK constraint `case_timeline_events.event_type` ne permet pas un type libre `HS10_AUTO_INJECTION` ; on réutilise `manual_action` pour traçabilité sans pollution des actions ouvertes). **Test B `31efcc01` (HS10 unique `4403110000`) pourrait qualifier pour Option C si le contrôle SH6=440311 confirme absence de divergence DD/TVA entre candidats HS10** — vérification à fournir dans le futur plan de patch, pas affirmée a priori. **Séquence imposée** : Phase 1 = documentation backlog (présent lot, audit-only) ; Phase 2 = micro-patch backend `HS10-AUTO-INJECTION-GUARD` Option C **isolé** (build-case-puzzle uniquement) ; Phase 3 = tests réels sur dossiers neufs ; Phase 4 = `HS10-SUGGESTION-UI-SELECTOR`. **Interdictions strictes phase audit** : aucune modif `build-case-puzzle`, `run-pricing`, `quotation-engine`, PAD-NST, Railway, migration, `src/`, `config.toml`. **Interdictions strictes phase 2 patch** : ne pas livrer simultanément avec UI selector (sinon impossible d'attribuer un bug au backend guard, à `set-case-fact`, à la fermeture de gap ou au selector). |
