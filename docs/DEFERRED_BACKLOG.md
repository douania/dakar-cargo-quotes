...
Dernière mise à jour : 2026-05-08 — **🔴 PAD-NST-2E-B-R2 RÉOUVERT : réconciliation DB active montre 9 extras, 9 manquantes, 16 écarts confidence, 5 écarts evidence_level, orphan group|15.1|T02. R2 non appliqué à la DB active. R3 requis. C-D implémentation bloquée. C-D-SPEC non invalidée.**
...
| PAD-NST-2E-B-R2 | Données | **🔴 RÉOUVERT** | P0 | PAD-NST-2E-B | 2026-05-08 | Réconciliation DB active vs expected_rules échouée : 9 extras, 9 manquantes, 16 conf mismatches, 5 evidence mismatches, orphan `group\|15.1\|T02`. R2 non appliqué à la DB active. | R3 requis. Voir `PAD_NST_2E_B_R3_FORENSIC_REPORT.md`. Ne pas corriger isolément. |
| PAD-NST-2E-B-R3 | Données | **🔴 REQUIS** | P0 | PAD-NST-2E-B-R2 | 2026-05-08 | GO CTO requis. Étapes : (1) export forensic CSV pré-R3, (2) migration data-only via `pad_nst_2e_b_r2_corrective.sql` versionné, (3) réconciliation 10 métriques = 0/88. | Purge + réimport depuis expected_rules. Aucun UPDATE isolé. Aucune reconstruction manuelle. Voir `PAD_NST_2E_B_R3_FORENSIC_REPORT.md`. |
...
| PAD-NST-2E-C-D | UI opérateur — implémentation | **🚫 BLOQUÉ — R3 requis** | P2 | PAD-NST-2E-B-R3 | 2026-05-08 | Bloqué jusqu'à R3 validé (10 métriques = 0/88). C-D-SPEC non invalidée. Addendum R1 non invalidé. | Implémentation Lovable uniquement après R3 clos + GO CTO séparé. |
| PAD-NST-2E-C-B-LOG | Migration audit log | **🚫 NO-GO temporaire** | P2 | PAD-NST-2E-C-B | 2026-05-08 | GO CTO séparé requis. Déblocage : spec audit validée + GO CTO + migration dédiée. Interdit de créer dans C-B. | Création table `pad_recommendation_audit_log` : traçabilité recommandations NST émises + décisions opérateur (accepted/rejected/modified). Migration data-only dédiée. Schéma indicatif dans `PAD_NST_2E_C_A_RUNTIME_PLAN.md` § 7. |
