...
Version: 1.9.1
Phase: EQ1.2 + CL1 + PAD + PAD-1 + PAD-ADMIN-UI + Magasinage DT + COCKPIT-5-P1 + SOURCE-GUARD + ORCH-SYNC-2 + PRICING-AUDIT-1 + CARRIER-PORT-TAX-1B-A + CLIENT-GAP-POLICY-FIX + P1-A + P1-B + P1-C + P2-A + P2-B + P2-C + P2-D Lot 2 + P0-3
Latest patch: P0 n°3 — resserrage préremplissage email partenaire (PURPOSE_INCLUDES freight resserré, fallback automatique supprimé, confidence absente = low)
Date: 2026-04-11
...
- Phase TARIFF-PROVENANCE Lot 1 (2026-04-24): Gouvernance de provenance tarifaire formalisée. Colonne `evidence_level` sur `port_tariffs`, `carrier_billing_templates`, `local_transport_rates`. **5 niveaux** : `official` (autorité réglementaire publiée — PAD, État, douane, arrêté DPW), `validated_internal` (document fournisseur fiable — Hapag/CMA/ONE PDF — **OU** validation interne SODATRA signée ; deux sous-cas non scindés à ce stade — voir `VALIDATED-INTERNAL-SUBLEVELS-SPLIT` au backlog), `observed` (facture/cotation isolée), `historical_only` (audit aveugle, dérivé statistique), `to_confirm` (placeholder). Niveau additionnel `client_override` sur `local_transport_rates` (utilisé Lot 2). **Whitelist runtime** : `evidence_level IN ('official','validated_internal')`. Filtres appliqués dans `quotation-engine` (port_tariffs L967, carrier_billing_templates L997), `price-service-lines` (DPW THC L626), `generate-response` (L1383+L1415). **Non touchées** : `run-pricing` (PAD/Dakar Terminal stabilisés, requêtes filtrées par `provider='PAD'` officiel pur), `analyze-attachments`, `data-query`, `data-admin`. `port_tariffs` reste structurellement hétérogène — refonte différée : `PORT-TARIFFS-NATURE-SPLIT`.
