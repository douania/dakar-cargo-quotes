...
| PAD-V5-B | 2026-06-03 | Phase B opérateur V5 shadow close : B1 UI read-only suggestion, B2.0 source CCC `pad_v5_shadow`, B2.1 Edge Function `create-pad-v5-classification-candidate`, B3 bouton UI “Créer candidat à valider”. Runtime validé sur fixture dédiée. V5 reste non-pricing, non-propagation, non-auto-accept. |
| PAD-V5-C2 | 2026-06-03 | Recovery runtime après Phase B : C2A a déployé `create-pad-v5-classification-candidate` après blocage 404 NOT_FOUND ; C2B a rejeté les CCC fixtures B3 via le workflow opérateur officiel MAP-5B. V5 reste suggestion-only, non-pricing, non-auto-propagation. Aucun changement migration/RLS/pricing. |
