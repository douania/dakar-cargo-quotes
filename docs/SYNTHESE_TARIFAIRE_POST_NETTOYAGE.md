...
5. **PAD-NOM-2 (2026-05-07)** — 324 alias `official_nomenclature` injectés dans `pad_designation_aliases` depuis le barème officiel PAD 2006. 9 catégories créées (P01–P05, T06, T08, T10, T11). 19 catégories PAD désormais couvertes. La résolution automatique des désignations marchandise passe de 60 à 384 alias. Les 4 lignes `operator_review_required`, 41 alias secondaires, et les conflits cross-category restent différés (voir `docs/DEFERRED_BACKLOG.md`).
