...
it('mixed cargo: proposes provisional standard estimates for 18t/15t and excludes declared DG', () => {
  ...
  expect(result.groups.every(g => g.max_payload_kg === null && !g.ordinary_transport && g.standard_estimate_only && g.unknown_danger_base_only)).toBe(true);
  expect(result.groups.every(g => String(g.qualification_source).includes('≤ 18 000 kg'))).toBe(true);
  expect(JSON.stringify(scope)).toBe(before);
});
it('keeps an unqualified heavier lot outside the active registry without blocking a standard lot', () => {
  const result = proposeTransportGroups({ cargo_units: [unit(), unit({ unit_ref: 'lot-heavy', gross_weight_kg: 18001,
    dangerous_goods: false })] });
  expect(result.groups.map(group => group.unit_ref)).toEqual(['lot-2']);
  expect(result.excluded).toEqual([expect.stringContaining('lot-heavy : poids supérieur')]);
});
