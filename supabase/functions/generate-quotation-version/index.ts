...
// Empêche qu'un snapshot contenant des lignes TO_CONFIRM soit
// stocké comme `firm`. Préserve `partial` et `provisional`.
// Pure function : testable sans Deno.serve.
// ─────────────────────────────────────────────────────────────
type QQLevel = "firm" | "provisional" | "partial";
type QQReason = { code: string; message: string; field?: string };
type QQ = {
  level: QQLevel;
  reasons: QQReason[];
...
export function resolveSnapshotQualification(
  ...
  // provisional préservé, merge reason si TO_CONFIRM
  ...
  // firm explicite OU absent/invalide
  if (hasToConfirm) {
    return {
      level: "provisional",
