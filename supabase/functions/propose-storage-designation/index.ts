...
export async function handleRequest(req: Request, deps = dependencies): Promise<Response> {
  ...
  try {
    ...
    const access = await db.from("quote_cases").select("id").eq("id", b.case_id).maybeSingle();
    ...
    if (scenario.error || !scope || scenario.data?.superseded_by_scenario_id || ["blocked", "superseded", "promoted_to_final"].includes(scenario.data?.status ?? "") || scope.schema_version !== 2 || scope.transport_mode !== "MARITIME" || scope.movement_direction !== "IMPORT") return errorResponse("Scénario maritime import actif requis", 422);
    ...
    const catalog = compatibleStorageCatalog(unit, designations.data as Row[]);
    const description = redact(unit.scenario_basis.slice(0, 1000));
    ...
    // Even a validated alias must be reviewed against this shipment's context.
    {
      try {
        const ai = await deps.ai([
          ...
          { role: "user", content: JSON.stringify({ target: storageContext(unit), movement_direction: scope.movement_direction, transport_mode: scope.transport_mode,
            context: units.map(storageContext), lexical_candidates_to_verify: candidates,
    ...
    return jsonResponse({ scenario_id: scenario.data!.id, scope_hash: scenario.data!.scope_hash, unit_ref: b.unit_ref,
      description, source: "Description et caractéristiques du lot, contexte des autres lots du scénario sélectionné (hypothèses). E-mails originaux et photos non relus ; référentiel Dakar Terminal, application DPW sous hypothèse.",
