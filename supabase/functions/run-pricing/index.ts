...
/**
 * Read service.overrides fact from a facts map.
 * Handles value_json as object or double-encoded JSON string.
 * Returns safe { add: [], remove: [] } on any failure.
 */
function readOverridesFromFacts(
  ...
  try {
    ...
    return {
      add: sanitize(parsed.add),
      remove: sanitize(parsed.remove),
    };
  } catch {
    return empty;
  }
...
/**
 * Resolve effective service keys from a package + overrides.
 * Order: base package keys, minus remove, plus add (no duplicates).
 */
