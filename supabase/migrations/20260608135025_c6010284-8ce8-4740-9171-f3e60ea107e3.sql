-- PAD-EXPORT-PROPAGATE-SAFE-1
-- Replace only public.propagate_classification_candidate_to_fact(uuid,text).
-- Adds an export-safe branch for accepted PAD export AI suggestions.
-- No RLS, GRANT/REVOKE, supersede_fact, UI, run-pricing, or price-service-lines changes.
...
DECLARE
  ...
  -- PAD-EXPORT-PROPAGATE-SAFE-1
  v_export_tariff_exists   boolean;
BEGIN
  -- 1. Validation entree
  ...
  SELECT id, case_id, candidate_kind, candidate_value, source, status, is_current, evidence, confidence
    INTO v_candidate
  ...
  -- 8. Niveau A (evidence) - bypasse pour pad_category (MAP-8B)
  ...
  -- 9. Niveau B (quote_facts) - bypasse pour pad_category (MAP-8B)
  ...
  -- 10. Conflit cle idempotence (autre candidat) - global, inchange
  ...
  -- ===== MAP-8B : bloc specifique pad_category =====
  IF v_candidate.candidate_kind = 'pad_category' THEN

    -- ===== PAD-EXPORT-PROPAGATE-SAFE-1 branch =====
    IF v_candidate.source = 'ai_suggestion'
       AND v_candidate.evidence->>'operation_type' = 'EXPORT'
       AND v_candidate.evidence->>'cargo_type' = 'CONTENEUR'
       AND v_candidate.evidence->>'map8b_export_safe' = 'false' THEN
      ...
      IF NOT v_containers_present THEN
        RETURN jsonb_build_object('ok', false, 'code', 'invalid_input',
          'details', jsonb_build_object('reason','unsupported_pad_export_scope',
      ...
      SELECT EXISTS (
        ...
        WHERE provider='PAD' AND category='DROIT_PASSAGE'
          AND operation_type='EXPORT' AND cargo_type='CONTENEUR'
          AND classification = v_value AND is_active = true
      ) INTO v_export_tariff_exists;

      IF NOT v_export_tariff_exists THEN
        RETURN jsonb_build_object('ok', false, 'code', 'invalid_input',
          'details', jsonb_build_object('reason','export_pad_category_tariff_not_found',
                                        'pad_category', v_value));
      END IF;

      -- C. Replay pad_category strictement current
      ...
      IF v_existing_pad_cat_fact IS NOT NULL THEN
        UPDATE public.commodity_classification_candidates
        SET evidence = COALESCE(evidence, '{}'::jsonb) || jsonb_build_object(
              'propagated_fact_id', v_existing_pad_cat_fact,
              'propagated_at', now(),
              'propagation_idempotency_key', p_idempotency_key
            )
        WHERE id = p_candidate_id;
        ...
        RETURN jsonb_build_object(
          'ok', true,
          'fact_id', v_existing_pad_cat_fact,
          'rate_fact_id', NULL,
          'candidate_id', p_candidate_id,
          'fact_key', 'cargo.pad_category',
          'idempotent', true,
          'replay_source', 'quote_facts_export_safe'
        );
      END IF;

      -- D. Propagation cargo.pad_category uniquement
      v_value_json := jsonb_build_object(
        'origin', 'PAD-EXPORT-PROPAGATE-SAFE-1',
        ...
        'scheme', NULL,
        'map8b_export_safe_branch', true,
        'map8b_emits_rate', false,
      ...
      SELECT public.supersede_fact(
        ...
        p_source_excerpt := '[PAD-EXPORT-SAFE] propagate candidate ' || p_candidate_id::text,
        p_confidence     := 1.0
      ) INTO v_new_fact_id;
      ...
      INSERT INTO public.case_timeline_events (
        ...
        jsonb_build_object(
          ...
          'rate_fact_id', NULL,
          'status',       'done',
          'scope',        jsonb_build_object('operation_type','EXPORT','cargo_type','CONTENEUR'))
      );
    ...
    -- E. Propagation cargo.pad_category (creer si absent, sinon reutiliser)
    v_value_json := jsonb_build_object(
      'origin', 'MAP-7B',
      'previous_map6_design', 'commodity/pricing keys deprecated',
      'propagated_from', 'commodity_classification_candidates',
  ...
  -- 11. Branches non-pad_category (hs10_uemoa) - flux original inchange
  v_value_json := jsonb_build_object(
    'origin', 'MAP-7B',
    'previous_map6_design', 'commodity/pricing keys deprecated',
    'propagated_from', 'commodity_classification_candidates',
