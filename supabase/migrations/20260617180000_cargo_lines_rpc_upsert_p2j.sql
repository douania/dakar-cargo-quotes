...
CREATE OR REPLACE FUNCTION public.upsert_cargo_equipment(
  ...
  p_equipment_type text DEFAULT NULL,   -- obligatoire (validé runtime) ; DEFAULT requis par contrainte PG sur l'ordre des paramètres
  p_quantity integer DEFAULT NULL,      -- obligatoire (validé runtime) ; idem
  p_status text DEFAULT 'to_confirm',
  p_source_quote_request_line_id uuid DEFAULT NULL,
  p_source_email_id uuid DEFAULT NULL,
  p_source_excerpt text DEFAULT NULL
...
BEGIN
  ...
  -- Cohérence tenant : si rattaché à une ligne, elle doit exister dans CE dossier.
  ...
  -- ── Verrou transactionnel sur la clé logique d'idempotence ──
  -- (case_id, cargo_line_id nullable, lower(trim(equipment_type)),
  --  source_quote_request_line_id nullable). COALESCE neutralise les NULL dans
  -- la clé textuelle ; séparateur '|' pour éviter les collisions de
  -- concaténation. Libéré en fin de transaction (xact).
  ...
  -- ── Recherche d'idempotence (NULL-safe via IS NOT DISTINCT FROM) ──
  SELECT id INTO v_existing_id
...
-- =====================================================================
-- GRANTS SÉCURITÉ
--   Révocation de l'EXECUTE PUBLIC accordé par défaut à la création, puis
--   grant exclusif à service_role. anon / authenticated : AUCUN EXECUTE.
-- =====================================================================
