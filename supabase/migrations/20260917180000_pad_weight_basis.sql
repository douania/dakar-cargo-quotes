-- LOCAL ONLY. Add a typed commercial weight basis; no client fact or tariff changes.
begin;
alter table public.pad_group_confirmations
 add column weight_basis text not null default 'confirmed',
 add column weight_reservation text not null default '',
 add constraint pad_weight_basis_check check (
  (weight_basis='confirmed' and weight_reservation='') or
  (weight_basis='provisional' and action='confirm' and length(btrim(weight_reservation)) between 10 and 2000)
 );
